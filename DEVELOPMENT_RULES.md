...
（含生产清单 `-r requirements.txt` + pytest==9.1.1 + PyYAML==6.0.3），
ci.yml / verify.yml 同源引用；pytest 不再裸装最新版。
生产依赖直连 import 但历史仅靠传递依赖带入的包（SQLAlchemy / alembic / pypinyin）
已在 requirements.txt 显式钉版。事故背景：API 推送漏掉 1 个提交导致远端
15 轮红 CI 而本地全绿——推送后必须跑 `python3 scripts/verify_remote_sync.py`。
...
- 仓库 `tests/` 从 v2 时期的 3 个文件增长到 2026-08-28 的 **265 个文件 / 945 个用例**，2026-09-12 已达 **1500 个用例**——A9 是这套增长的核心推手，继续保持"每个新增业务函数至少 1 个失败测试"，逼迫 AI 写可验证的代码。
...
## 七、防反复 BUG 规则（R1–R8，2026-08-28 新增，2026-09-20 补 R8）
...
> 与 `AGENTS.md §七 R8` 同一条文，此处为执行侧细化。R 系列以人工/AI 自检为主。

- **铁律**：一次修复的**净收益必须为正**。带病交付（修了 A 坏了 B）比不修更糟——
  未修的 BUG 在台账里看得见，新引入的 BUG 要等用户在现场踩到才发现。
- **禁止**：①为让测试变绿而放宽/删除既有断言（改测试迁就代码 = 伪修复）；
  ②只跑自己新增的用例就宣称完成；③明知引入新失败仍推送；
  ④口径未定/涉及面过大/无法验证时硬改业务账务逻辑。
- **修复"完成"的六项判据（缺一不得向用户报完成）**：
  1. **修复前基线**：先跑受影响模块全量，记录本来就红的项；
  2. **回归锁**：新增针对本 BUG 的用例；
  3. **回退验证**：回退修复后新用例**必须失败**（否则是无效锁）。
     回退用文件备份法（`cp` 备份 → `git checkout --` → 跑测试 → `cp` 恢复），
     **不要用 `git stash`**（本环境 2026-09-20 曾因 stash 被 SIGTERM 中断损坏仓库）；
  4. **全量 + 相邻**：受影响模块全量 + 相邻模块回归 + `scripts/lint_wms_rules.py` 0 违规；
  5. **CI 实证**：本地无法真实验证者（Android/Kotlin 等）必须等三工作流转绿才算完成；
  6. **登记闭环**：台账写清根因/危害/修复/回归/**生效确认**（A13）。
- **修不好就别修**：无法在可控范围内修好时**只登记**（建议方案 + 生效确认「待确认」），
  代码一行不动，等用户拍板后单独 atomic action 实施。
- **已引入新失败**：立即 `git revert` 回滚，回到"已登记未修"状态重新评估，
  不得带红灯继续下一任务（§三 CI 全绿门禁）。
- **实证**：2026-09-20 两次"本地看着绿就推送"——①`"$label前一天"` 中文标识符导致
  `compileReleaseKotlin` 失败；②归档误迁 `fix_inventory_check_columns.bat`
  （测试用 `ROOT / "xxx.bat"` 拼接路径，字符串 grep 查不到）导致 CI 单测红灯。
...
| 2026-09-20 | v5：新增 **R8**（修 BUG 不得引入新 BUG；修不好就别修）——修复"完成"六项判据（修复前基线 / 回归锁 / **回退验证** / 全量+相邻 / CI 实证 / 登记闭环）、禁止改测试迁就代码、引入新失败立即 revert、无法修好只登记；第七节标题 R1–R6 → R1–R8（R7 已于 09-19 补登） | AI + SIX2090 |
