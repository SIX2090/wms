...
> 阅读说明（2026-07-26）：本文同时保留早期方案、V2 方案和阶段实施记录。带“实施记录”的章节属于历史证据，不代表当前待开发事项；当前状态必须以文末交付门槛、实际代码和验证脚本为准。新增销售任务应先核对现有实现，避免按旧版缺口重复开发。
...
## 阶段 19 实施记录（2026-07-19）：真实账号与售后数据验收盘点

- 管理员真实浏览器验收通过：用户管理、售后出库管理、销售核心页面均能正常打开，中文页面无错误页。
- 生产库只读盘点结果：当前仅有 `admin` 管理员账号，没有 `sales`、`warehouse` 或普通用户业务账号；售后出库单总数为 0，原销售订单来源和原销售出库来源均为 0，暂无生产售后数据可对账。
- 未擅自创建账号、设置或生成密码，未伪造售后单据或修改生产数据。待业务管理员提供已授权的真实账号、或明确授权账号开通流程，并在有真实售后数据后继续多角色和售后对账验收。

**验证**：Playwright 真实浏览器已检查 `/user`、`/after_sale_out`、`/sales/dashboard`、`/sales/exceptions`、`/sales`、`/sales/report`、`/sales/reconciliation`；页面均正常渲染。角色权限自动化回归仍为通过，但真实多角色和生产售后对账状态为“待业务数据/账号”。

## 阶段 20 实施记录（2026-07-19）：10 用户 10 组全 WMS 浏览器验收

- 在独立 `app/instance/test_full_flow_10users.db` 中创建 10 个测试用户：管理员、采购、销售、仓库和只读角色；测试密码固定透明，不生成随机密码，不修改现有业务库账号。
- 生成 10 组采购订单、采购入库、库存、销售订单、销售出库和售后出库草稿；售后草稿均关联原销售订单及原销售出库单，保持待完成，不扣库存。
- 真实浏览器完成销售账号登录、销售订单新建/保存/确认、销售出库草稿下推；仓库账号查看来源出库单、处理异常提示并完成出库；售后详情来源下钻和销售对账页面均通过。
- 发现并修正测试数据构造器的两个问题：部分出库数量变化时同步明细金额；售后草稿同步单据合计金额。修正后部分发货和售后金额全部对账通过。

**验证**：`python scripts/verify_full_flow_10users_10groups.py` 187/187 业务步骤通过；10 用户、10 售后来源草稿、角色边界全部通过；Playwright 真实浏览器页面和操作链路通过；服务验收后已恢复原业务数据库，`/login` 返回 200。测试库与业务库隔离。
