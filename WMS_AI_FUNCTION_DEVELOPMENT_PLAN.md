...
| 13 | AI-R13 | 下一项 | Agent 预算、取消、熔断和并发控制 | R01、R02 | R16、R17 |
...
开始前必须检查最大步骤、最大耗时、最大工具调用、截止时间、取消、并发互斥、失败重试来源、Provider 熔断和等待人工状态是否已关联台账任务编号；无无限循环；超预算、越权和故障安全停止；重试保留原证据；自动提交业务单据次数为 0。
