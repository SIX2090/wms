...
`AI-R17-F02：连续七天真实上线验收`。

AI-R01～R17 的基础能力已经完成。AI-R17-F01 真实用户白名单灰度与一键回滚闭环已完成（用户白名单接入主流程、4 种灰度模式、灰度拒绝审计、Provider 降级、一键关闭/恢复）。下一阶段继续推进 F 系列子项，把现有能力从"代码和测试完成"推进到"真实用户可灰度、可衡量、可回滚、可运营"。
...
| P0 | AI-R17-F02 | 待开发 | 连续七天真实上线验收 | AI-R17-F01 | 真实指标采集、确认状态、验收证据包 | 四项上线违规指标连续七天为 0；验收数据可复算 |
