...
| AI-WH-BACKFILL-001 | 2026-08-18 | `cb74962` + `365a099` | `scripts/backfill_document_warehouse.py`（历史无仓库采购入库单/领料单及关联流水回填默认仓库，dry-run 默认、`--apply` 写库、幂等可重跑）、`tests/test_backfill_document_warehouse.py`（5 项回归）、`app/routes/in_order.py`（`revert_in_order` 补 `StockTransaction` import，修 BUG-2026-08-18-006 NameError 500） | `python3 -m pytest -q tests/test_backfill_document_warehouse.py tests/test_bug_2026_08_16_009_revert_in_order_warehouse_level.py tests/test_bug_2026_08_17_002_revert_in_order_without_warehouse.py`、`python3 scripts/lint_wms_rules.py`、`python3 scripts/verify_wms_bugs.py`、`python3 -m pytest tests/ -q` | 通过（回填专项 5 passed、反提交专项 6 passed；lint 0 违规；verify 无 FAIL；全量 591 passed） | 生产库执行：先 `python3 scripts/backfill_document_warehouse.py`（dry-run 核对明细）→ 确认后 `--apply` 写库；要求已配置默认仓库 |
