...
| 83 | BUG-2026-08-18-001 | 已完成 | 入库 Excel 导入、其他入库编辑/导出/采购报表及批量反提交与仓库和业务类型口径一致性修复 | 提交 `eb9a099` | 验证：`pytest tests/ -q`（579 passed）、lint_wms_rules 0 违规、lint_no_raw_post_fetch 通过、verify_wms_bugs 回归通过；回归测试录入 `tests/verify_app_py_split_batch_import.py`/`verify_app_py_split_export.py`/`verify_app_py_split_report.py`，覆盖导入字段保真、其他入库导出业务类型过滤与客供字段、采购报表业务类型隔离、多仓批量反提交仓库级库存校验 |
