...
| 103 | PRINT-TEMPLATE-F05 | 进行中 | 打印模板在线 Excel 编辑增强（需求 2026-08-25：在线编辑要有常规工具栏能编辑样式并支持打印预览）：A1 网格序列化输出样式/尺寸（`serialize_print_template_grid` 每格输出 style：加粗/斜体/下划线/字体/字号/颜色/背景色/对齐/换行/边框，sheet 输出 col_widths/row_heights，空值带边框格不再丢失，向后兼容）；A2 `apply_print_template_grid` 支持 styles/col_widths/row_heights 回写 + 白名单校验 + pydantic 扩展；A3 apply 支持 merges/unmerges + del_rows 与合并区相交守卫 + 删行后合并平移；A4 编辑器前端常规工具栏（字体/字号/B/I/U/颜色/对齐/边框/合并/列宽行高）+ 拖选选区；A5 预览数据接口 `GET /{prefix}_print_template/<id>/preview_data`（类型感知示例数据）；A6 编辑器内打印预览（示例数据填充 + @media print + window.print()） | 无 | A1 完成：提交见 git log；验证 `pytest tests/test_print_template_grid_styles.py tests/test_print_template_grid_editor.py -q`（13 passed）；A2-A6 待续 |
