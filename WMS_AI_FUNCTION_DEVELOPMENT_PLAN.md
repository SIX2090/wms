...
- `start_wms_auto.bat` 简化为委托给 `start_wms_offline.bat`，避免与 `run_server.py` 内置 `auto_update` 调用重复触发；保留向后兼容以支持已建立桌面快捷方式的用户。
...
3. `start_wms_auto.bat` 委托给 `start_wms_offline.bat`，不再直接执行 `auto_update.py`，避免重复触发。
...
- 任何启动路径（`wms.bat` / `wms.cmd` / `start_wms_offline.bat` / `restart.py`）都触发 `auto_update`。
