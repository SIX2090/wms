...
| 17 | AI-R17 | 下一项 | 真实用户灰度、回滚演练和上线验收 | R09～R16 | 正式发布 |
...
开始前必须规划管理员、仓库主管和指定采购员灰度名单；记录耗时、修正、误判、失败和回退；演练 Provider 故障、权限攻击、重复请求、关闭 AI 和恢复配置；连续一周越权成功 0、重复草稿 0、自动提交 0、低置信度未确认建单 0；10 分钟内关闭 AI 并恢复配置。
