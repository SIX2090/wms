...
| 85 | PRINT-ROUTING-F01-P5 | 已完成 | 手工新增打印机：不依赖打印代理自动注册，工作站卡片新增「新增打印机」按钮，支持手工录入打印机名称、系统名称（留空自动取名称）、类型和启用状态；后端新增 POST /print_routing/printers 路由（pydantic PrinterCreateRequest 校验），同一工作站下同 system_name 重名拒绝 400 | 无 | 提交 `e69655a`；验证 `pytest tests/test_print_routing_admin.py -q`（7 passed，含 test_printer_add_manual）、lint_wms_rules 0 违规 |
