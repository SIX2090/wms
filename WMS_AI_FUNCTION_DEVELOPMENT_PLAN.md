...
| 96 | PRINT-ROUTING-F01-P6 | 已完成 | 内置本机打印代理（SERVER-AUTOPRINT-01）：WMS 服务进程内嵌后台代理线程（app/local_print_agent.py），启动期幂等 ensure_builtin_print_workstation（独立 sqlite 连接、WMS_NO_DB_TOUCH 安全、内置 LOCAL-SERVER 工作站 + print 系列表自愈补建），常驻心跳上报本机打印机 + 认领定向到内置工作站的待打印任务 + Edge/Chrome kiosk-printing 静默出纸（临时切默认打印机实现定向、出纸后恢复、WMS_LOCAL_PRINT_AGENT=0 可关）；routes/print_queue.py 提取 sync_workstation_printers / build_agent_print_url / mark_job_printed / _resolve_job_assignment（在线规则 > 离线规则 > 内置代理兜底 > 非定向）作为 v1 路由与内置代理共用的单一事实来源，消除两处派发逻辑重复；app.py / run_server.py 接线（启动期 ensure + 服务启动后代理线程）；tests/test_local_print_agent.py 15 项回归覆盖枚举解析/默认打印机读写/kiosk 浏览器定位/进程管理/ensure 幂等/兜底派发/单轮心跳+认领打印/启动开关 | 无 | 提交 `bcacf3ab`；验证 `pytest tests/test_local_print_agent.py tests/test_print_queue.py tests/test_print_agent_api.py tests/test_print_agent_script.py tests/test_print_routing_admin.py tests/test_ensure_print_job_columns.py -q`（96 passed）、lint_wms_rules 0 违规、pre-commit 全过；推送 `83aeabed..bcacf3ab  main -> main`，远程 `bcacf3ab053ff38d1e240335e3f0e96a5a06f38f` 与本地一致；GitHub 提示 lint-and-test status check 等待 CI 跑完，不阻塞推送 |
...
| 102 | PRINT-TEMPLATE-F04 | 已完成 | 统一打印模板设计中心（需求 2026-08-25：所有打印模板支持 Excel 在线编辑，参考简道云打印模板，单据/报表/列表/标签都在一个中心设计）：A2 引擎支持 `{img_barcode:item.*}`/`{img_qrcode:item.*}`/`{img_*:order.*}` 图片占位符（填充时嵌入 600DPI 条码/二维码 PNG，数据为空回退文本，上传/在线编辑白名单同步放行）+ `doc_print_excel.py` LABEL_EXCEL_PRINT_TYPES（material_label 内置物料标签模板+启动幂等同步）+ `render_label_excel_print`；A3 `/label/batch_print_excel?ids=&template_id=` 打印出口（ptoken 绑定 label）+ 批量标签页 Excel 模板下拉/下载按钮/设计入口；A4 报表统一模板打印出口 `/report/<report_type>/print_excel`（复用 `_build_report_filters`+仓库必填+`_build_report_payload`，target_code=`report_<type>` 按 columns 动态生成内置模板，首次打印自动登记进模板中心幂等）+ 报表页「按模板打印」入口 + 引擎 `_resolve_path` 支持 dict 行；A5 模板中心升级（标签分类、「在线新建」免上传直达编辑器、target_code 分类联动 datalist、入库/出库/标签设计器统一入口卡片）；A6 `/report/stock/print` 支持 template_id/默认模板记录走 stock_query 模板渲染（无模板回退原硬编码导出，仓库隔离不变）+ 库存查询页模板下拉 | 无 | 提交 `380a8cbe`（A1 台账补登 F03+F04 登记）、`b95b74bd`（A2）、`98465a2d`（A3）、`7eaa7e41`（A4）、`f4f97589`（A5）、`891c64de`（A6）；验证：全量 `pytest tests/ -q` 859 passed（含新增 tests/test_print_template_label_excel.py 9 项、test_label_excel_print_route.py 5 项、test_report_excel_print_route.py 8 项、test_print_template_center.py 6 项、test_stock_query_template_print.py 4 项）、`lint_wms_rules.py` 0 违规、`lint_no_raw_post_fetch.py` 通过、`verify_wms_bugs.py` 回归通过、pre-commit 全过；真实服务冒烟：启动同步 14 个内置模板（含 label/material_label），`/label/batch_print_excel`、`/report/inventory/print_excel`（自动登记 report_inventory）、`/report/stock/print`（模板渲染）、`/global_print_template/<id>/edit|grid`、`/print_templates/create_blank`（302 直达编辑器）全部实测通过 |
| 103 | PRINT-TEMPLATE-F05 | 已完成 | 打印模板在线 Excel 编辑增强（需求 2026-08-25：在线编辑要有常规工具栏能编辑样式并支持打印预览）：A1 网格序列化输出样式/尺寸（`serialize_print_template_grid` 每格输出 style：加粗/斜体/下划线/字体/字号/颜色/背景色/对齐/换行/边框，sheet 输出 col_widths/row_heights，空值带边框格不再丢失，theme/indexed 色不输出，向后兼容）；A2 `apply_print_template_grid` 支持 styles/col_widths/row_heights 回写（语义「有值=设置/显式 null=清除/缺失=保持」）+ 样式键白名单校验（字号 6-72、#RRGGBB、对齐/边样式枚举）+ pydantic `_CellStyleModel`；A3 apply 支持 merges/unmerges（锚点/相交/越界校验）+ del_rows 与合并区相交一律 400（防 openpyxl delete_rows 不平移合并区坏文件）+ 删行后合并自动平移；A4 编辑器前端常规工具栏（字体/字号下拉、B/I/U、字体色/背景色及清除、水平/垂直对齐、自动换行、全部/外侧/无框线、合并/取消合并、列宽/行高）+ 拖选选区（单击选中/按住拖选/行列头选整行整列）+ 样式即时渲染 + DIFF 提交扩展；A5 `app/print_preview.py` build_preview_context（按 DOC/TABLE/LABEL 注册表生成类型感知示例数据，入库/出库硬编码示例，未知类型回退）+ `GET /{prefix}_print_template/<id>/preview_data`；A6 编辑器内打印预览（含未保存改动、{item.*} 按 3 条示例明细扩展、图片占位符虚线框示意、未命中占位符灰色保留、@media print 仅预览区可见 + @page A4 + window.print()） | 无 | 提交 `f3f9f33a`（A1）、`3219be6e`（A2）、`644e3fbe`（A3）、`5267e91c`（A4）、`ffcd3137`（A5）、A6 见 git log；验证：新增 tests/test_print_template_grid_styles.py（11 项）+ test_print_template_grid_merge.py（11 项）+ test_print_template_preview.py（7 项），全量 `pytest tests/ -q` 888+ passed、`lint_wms_rules.py` 0 违规、`lint_no_raw_post_fetch.py` 通过、pre-commit 全过；真实浏览器冒烟（Chromium + waitress QA 服务）：A4 工具栏渲染/选区/加粗切换/拖选合并/取消合并/保存后样式持久化 22 项全过，A6 预览显示/示例数据填充/明细行扩展/未保存改动进预览/占位符无残留/@media print 存在/返回编辑 14 项全过，无 JS 错误 |
...
| 107 | INV-AUDIT-003-FIX-01 | 已完成 | Android 原生端 `/api/stocktake` 扫码盘点账面改仓库级取数（INV-AUDIT-003 遗漏修复，对应 BUG-2026-09-02-001）：`native_api.py` `native_api_stocktake` 在循环外一次性取 `warehouse_stock_map = get_warehouse_stock_quantities(wh_obj)`（避免逐行 N+1），行内 `system_stock` 默认值从全局 `material.stock` 改为仓库级 `warehouse_stock_map.get(material.id) or 0`，对齐 `mobile.py` scan_submit；客户端显式传值时仍沿用（向后兼容，Android ScanViewModel 固定传 null 必然走仓库级分支）。多仓库下此前把"全部仓库合计"当成单仓账面数，盘盈盘亏全错并生成错误调整草稿 | INV-AUDIT-003 | 回归 `tests/test_inv_audit_003_fix01_native_stocktake_warehouse_scope.py` 3/3 PASSED（T1 多仓取 B仓账面 40 差异 -2、T2 A仓账实一致不生成调整单、T3 单仓 fallback 不受影响）；`scripts/lint_wms_rules.py` 0 违规、`lint_no_raw_post_fetch.py` 通过。**发布边界**：后端改动，Android App 无需重新发版 |
| 108 | INV-AUDIT-003-FIX-01 遗留 | 待办 | `tests/test_inv_audit_003_mobile_scan_warehouse_scope.py` 单独跑 7/7 PASSED，但与 `verify_bug_2026_08_11_021` 等文件同批跑时 3 项失败（测试间 session/DB 状态污染，已在修复前 baseline 复现，非本次改动引入）。需拆为独立任务修复测试隔离 | INV-AUDIT-003-FIX-01 | 无 |
| 109 | INV-GUARD-001 | 已完成 | 扫码盘点跨盘点单重复调整护栏（对应 BUG-2026-09-02-002）：`app/app.py` `_create_adjustment_drafts_from_check_scan` 生成草稿前检查同仓库同物料是否已有未提交（pending）的扫码盘点调整草稿，有则拒绝并提示先处理存量草稿（提示含物料编码与 ADJ 单号）；不同物料/不同仓库/已提交草稿（合法二次盘点）放行；历史无仓库字段草稿按潜在冲突处理。手机端（mobile.py scan_submit）与 Android（native_api.py /api/stocktake）共用此函数，一处拦截两端生效 | 无 | 回归 `tests/test_inv_guard_001_stocktake_duplicate_adjustment_guard.py` 5/5 PASSED（TDD：红阶段 T1/T5 复现两入口重复盘点均放行生成第二张草稿，绿阶段全过）；既有扫码盘点回归 29/29 PASSED（fix01 3 + mobile_scan_warehouse_scope 7 + stocktake_warehouse_required + mobile_scan_draft_flow 22 中合计）；lint 0 违规。**发布边界**：后端改动，App 无需发版 |
| 110 | INV-REVERT-001 | 已完成 | 扫码盘点单作废（可回退）能力（对应 BUG-2026-09-02-003）：`app/app.py` 新增 `_void_check_scan(check_id, operator_id)`——写锁重读防并发、关联调整单已提交则拒绝作废（提示先反提交调整单）、级联删除 pending 调整草稿及明细（护栏随之解除可重盘）、单据软标记 status='void' 留痕、Bearer 请求按 operator_id 补写 OperationLog；双端点 `POST /mobile/api/check_scan/<id>/void`（会话/Bearer 双认证）+ `POST /api/stocktake/void`（按 check_no，Android 用）；`mobile_scan.html` 盘点历史条目新增撤销按钮（scan_submit 响应新增 check_id 字段）；`custom.css` 新增 .mobile-history-undo 样式 | 无 | 回归 `tests/test_inv_revert_001_check_scan_void.py` 7/7 PASSED（TDD 红阶段 7/7 失败确认，绿阶段全过）；既有扫码盘点回归 19/19 PASSED（guard 5 + fix01 3 + mobile_scan 7 单独跑 + 仓库必填 4；同批跑的 mobile_scan 3 项失败为任务 108 登记的既有测试污染，单独跑 7/7）；lint_wms_rules 0 违规；盘点页渲染 200 + script 块 node 语法检查通过。**发布边界**：后端+Web 手机端页面改动，Android App 下版本接入撤销 UI 时后端已就绪 |
| 111 | INV-BATCH-001-A | 已完成 | 盘点批次化模型层（Phase 2-A）：`InventoryCheck` 加 `frozen_at`（批次账面冻结时点，首次写入明细时设置，之后差异=实盘−冻结账面不随出入库漂移）；`InventoryCheckItem` 加 `counted_by`/`counted_at`（行级盘点归属，FK user.id + 单向 relationship，多人协作可判责）；`InventoryCheckScan` 加 `check_id`（挂钩批次 FK inventory_check.id + batch relationship，挂批次后由批次 complete 统一生成调整草稿，NULL=独立单模式向后兼容）；`auto_migrate_database` 补 4 列（全部可空，存量 NULL 语义与旧行为一致，无需回填） | 无 | 回归 `tests/test_inv_batch_001a_check_freeze_model.py` 3/3 PASSED（T1 三表新列存在/T2 完整 schema 重建法模拟老库补列+二次幂等（counted_by 带 FK 不能 DROP COLUMN，用重命名重建法）/T3 兼容不带新字段建单正常）；既有扫码盘点回归 26/26 PASSED（revert 7 + guard 5 + fix01 3 + 仓库必填 4 + mobile_scan 7 单独跑）；lint_wms_rules 0 违规。**发布边界**：仅模型层+迁移，行为不变，App 无需发版 |
| 112 | INV-BATCH-001-B | 已完成 | 盘点明细行级 upsert 与冻结快照（Phase 2-B，对应 BUG-2026-09-02-004）：`save_check_table` 全删重建 → 行级 upsert——已有行（按物料匹配）保留 system_stock 冻结值仅更新实盘/原因/差异，新增行取当前仓库级账面，提交集之外旧行删除；首次写入明细设 `check.frozen_at`（一经设置不再变更）；保存路径加 `_acquire_order_write_lock` 单据写锁防并发覆盖；`update_check_item` 加写锁并写入 `counted_by`/`counted_at` 行级归属。行为变化：已有行账面不再随重复保存刷新（冻结快照预期语义），如需最新账面重录某行须删行重加 | 无 | 回归 `tests/test_inv_batch_001b_check_freeze_upsert.py` 6/6 PASSED（TDD 红阶段 T1/T2/T3/T5 复现漂移与归属缺失，绿阶段全过；T4 账面变化后 complete 按冻结口径 57−60=-3 生成调整单而非 57−30=+27）；既有回归 62 项 PASSED（含 save_table 依赖的 warehouse_module 23 + audit_005 23 + 扫码盘点全族 26 项，mobile_scan 7 单独跑）；lint_wms_rules 0 违规。**发布边界**：后端改动，App 无需发版 |
| 113 | INV-BATCH-001-C | 已完成 | 扫码盘点挂钩批次（Phase 2-C）：`app/app.py` 新增 `_find_active_check_batch`（同仓库最新 pending 批次）与 `_apply_scan_to_batch`（行级 upsert——已盘行 counted_at 非空则拒绝并提示"已由谁何时盘点"防两人互覆、未盘行更新实盘与归属且账面基准保留、无行插入取扫码时点仓库级账面即时口径、空批次首扫设 frozen_at）；`mobile.py` scan_submit 与 `native_api.py` /api/stocktake 双入口检测批次：有批次挂 `check_id` 不再独立生成调整草稿（批次 complete 统一生成），批次写锁（`_acquire_order_write_lock`）防并发互覆，并发下批次失效（被完成/反提交）自动退化独立模式不丢盘点数据；无批次维持独立模式（立即生成草稿 + INV-GUARD-001 护栏）向后兼容。口径推导（实物守恒）：调整量 = 实盘 − Δ(t0→ti) − F = 实盘 − 该行盘点时点账面——PC 表格行用冻结口径（停机语义）、扫码行用即时口径（不停机语义），批次内各行基准各自自洽，complete 统一按 actual − system_stock 生成 | 无 | 回归 `tests/test_inv_batch_001c_scan_hooks_batch.py` 7/7 PASSED（T1 无批次独立模式锁定/T2 挂批次+不生成 CS 级草稿+行级归属+响应含批次号/T3 未盘行更新保账面基准/T4 重复扫码拒绝含已盘信息与批次号/T5 期间出库后新行取扫码时点账面（差异 -1 而非 -11）/T6 批次 complete 按各行口径生成调整单且 CS 无独立草稿/T7 跨仓库隔离）；既有回归 69 项 PASSED（同批 62 项：batch_c 7 + batch_b 6 + batch_a 3 + revert 7 + guard 5 + fix01 3 + 仓库必填 4 + warehouse_module 23 + audit_005 4 等 + mobile_scan 7 单独跑）；lint_wms_rules 0 违规。**发布边界**：后端改动，App 无需发版（响应新增 batch_no 字段，向后兼容） |
| 114 | INV-BATCH-001-D | 已完成 | Web 盘点单批次进度与回查（Phase 2-D）：`app/app.py` 新增 `_build_check_batch_meta(check)` 并注入 `_render_check_form`（batch_meta：frozen_at 冻结时点/已盘物料数（去重）/有差异行数/挂入的扫码盘点单列表——单号、日期、盘点人、提交时间、条数，最近 20 条+总数）；existing_items 行级 extra 补 `counted_by_name`/`counted_at`（行级归属回查，谁在何时盘了该行）；`check_detail` eager load `counted_by_user` 消除 N+1；`document_table_form.html`（模板改动，**生产需重启 WMS 服务生效**）——盘点单既有单据展示「盘点人 / 时间」只读列（hasCountedByCol 仅在 doc_type==check 且 order_id 时启用，thead/rowCells/tfoot 三处对齐，提交数据收集不受影响）+ 表格上方批次进度卡（冻结状态 badge/进度 badge/扫码记录表）。多人协作下主管可实时回查批次进度与逐行归属 | 无 | 回归 `tests/verify_inv_batch_001d_check_batch_progress.py` 3/3 PASSED（T1 空批次详情渲染进度卡+未冻结提示/T2 扫码挂批次后详情含冻结时点、已盘 1 物料、counted_by_name=admin、CS 扫码单号/T3 批次完成只读后归属与扫码记录不丢）；批次盘点全族 40 项 PASSED（batch_a 3 + batch_b 6 + batch_c 7 + guard 5 + revert 7 + fix01 3 + 仓库必填 4 + app_split 5？以实测 40 为准）；渲染详情页 6 个内联 script 块 node --check 全过；lint_wms_rules 0 违规。**发布边界**：后端+模板改动，需重启服务生效；Android App 无需发版 |
| 115 | INV-BATCH-002 | 已完成 | 盘点区域分行（多人分区/分仓盘点，物料分散多处汇总正确）：为满足现场"几个人分区盘点、同物料分处多区能统计正确"的诉求——`InventoryCheckItem`/`InventoryCheckScanItem` 加 area 列（auto_migrate 幂等补列，可空默认''=不分区单行向后兼容）；`_apply_scan_to_batch` 行键由物料改为 (material_id, area)——同物料不同区域各占一行、同区重复扫仍拒绝（防互覆）；mobile scan_submit 与 Android native_api stocktake 透传 area；差异草稿按物料净额合并——分区物料取"各区域实盘合计 − 仓库账面(只计一次)"，防止多区各行各扣一次账面造成差异翻倍（例：账面 20、A1 盘 12 + A2 盘 7 → 净 −1 而非 −21）；`mobile_scan.html`（模板，生产重启生效）盘点页新增"盘点区域/货架"输入（可选） | 无 | 回归 `tests/verify_bug_2026_09_03_005_check_area_split.py` 5/5 PASSED（T1 双区两行+area 落库/T2 同区重复拒绝/T3 完成净额单条 −1/T4 不填区域单行不回归/T5 两表含 area 列）；批次盘点 verify 全族各文件单独 7/7、6/6、5/5 通过；lint_wms_rules 0 违规。**发布边界**：后端 + 模板改动，需重启 WMS；Android 老版本不受影响（不传 area=不分区） |
| 117 | INV-WECHAT-MASTER-001 | 已完成 | 微信分享总开关（2026-09-04 业务诉求：微信分享暂时不用时一键整体停用）：`app/app.py` 新增 `_wechat_share_master_enabled()`（读系统设置 `wechat_share_enabled`，默认 '1' 开，兼容存量）；发送入口统一把关——`run_due_wechat_share_jobs` 定时入口关闭即跳过、`run_wechat_share_for_today` 任意触发（含手动）关闭即返回"总开关已关闭"提示、`_async_wechat_share_on_complete` 入库完成异步直推关闭即跳过；图片 30 天保留清理与发送无关不受开关影响。`wechat_share.py` 页面/保存路由接入：页面顶部新增醒目「微信分享总开关」勾选+状态 badge，保存时写系统设置（复用既有 /wechat_share/save 表单，FormData 提交未勾选即关）；`wechat_share.html` 模板改动需重启生效 | 无 | 回归 `tests/test_wechat_share_master_switch.py` 5/5 PASSED（默认开/0-1 切换/关闭时 run_* 拒绝/定时入口无日志/保存页面切换与 badge）；既有微信分享 verify 37/37 零回归。**发布边界**：后端+模板改动，拉取重启 WMS 生效，无需数据库迁移 |
| 118 | INV-BATCH-001-F | 已完成 | 盘点域迁移列启动期自愈补列 + 物料编辑级联统计降级（INV-BATCH-001-A/002 子修复，对应 BUG-2026-09-05-001）：`app/app.py` 新增 `ensure_inventory_check_columns()`（仿 `ensure_print_job_columns`，独立 sqlite 连接、独立于 `WMS_NO_DB_TOUCH` 迁移开关无条件执行、PRAGMA table_info 幂等），一次性覆盖盘点域全部新增列——`inventory_check.frozen_at`、`inventory_check_item.counted_by/counted_at/area`、`inventory_check_scan.check_id`、`inventory_check_scan_item.area`，列片段与 `auto_migrate_database()` 逐字一致；启动序列在 `ensure_stock_transaction_warehouse_id_column()` 之后调用。修复根因：迁移列只在 `auto_migrate_database()` 里 ADD，而 `start_wms_offline.bat` 等启动脚本默认 `WMS_NO_DB_TOUCH=1` 整体跳过迁移、`fix_db_columns.py` 又不含盘点表补列，存量库重启也补不上 → 物料编辑保存的盘点明细统计查询报 `no such column: inventory_check_item.counted_by` 打 500（同根因第 4 次复发：print_job / stock_transaction / excel_print_template / 本处，按 R6 一次性覆盖而非只补报错列）。`app/routes/material.py` 编辑保存的 5 项「仅统计条数、无冗余字段」查询改为可降级 `_append_related_count()`（失败只记 WARNING 并跳过该项提示），杜绝数据库与代码不同步再打成 500；`delete_material` 引用检查已有逐表 try/except 降级，无需改动 | 111 INV-BATCH-001-A / 115 INV-BATCH-002 | 完成日期 2026-09-05，提交 `b89dc310`。回归 `tests/test_ensure_inventory_check_columns.py` 5/5 PASSED（6 列全补齐/幂等/列齐全不重复 ALTER/空库缺表跳过/子进程 `WMS_NO_DB_TOUCH=1` 启动接线补列）+ `tests/test_bug_2026_09_05_001_material_edit_cascade_degrade.py` 3/3 PASSED（缺列时编辑仍 success 且改名落库/提示剔除降级项/正常路径仍统计「盘点单 N 条」）；`pytest tests/` 1162 passed 0 failed；`scripts/verify_wms_bugs.py` PASS；`lint_wms_rules --staged` 0 违规。**发布边界**：后端+数据库改动，拉取重启 WMS 服务即自动补列（只 ADD 可空列、不动存量数据），无需手工改库 |
| 116 | INV-BATCH-001-E | 已完成 | 移动盘点强制选单（业务决策：所有盘点结果统一记录到一张 PC 盘点单上，对应 BUG-2026-09-04-005）：`app/app.py` 新增 `_list_pending_check_orders(warehouse=None)`；新增 GET `/api/stocktake/check_orders`（native）与 `/mobile/api/check_orders`（H5）盘点单列表端点（仅 pending，可按仓库名称/编码过滤）；`native_api.py` /api/stocktake 与 `mobile.py` scan_submit(check) **强制校验 check_id**——缺省/不存在/已完结/与盘点仓库不一致一律 400，删除"无批次独立建 CS+草稿"旁路，明细统一 `_apply_scan_to_batch` upsert 进所选批次，返回 batch_no/inventory_check_id；`_void_check_scan` 补批次行回退（本 CS 新建行删除、PC 预置行被本 CS 补盘的复位为待盘、批次已完成拒绝作废、清空批次撤销冻结时点）；`scripts/verify_wms_bugs.py` BUG-NEW2-004 断言随新契约更新。旧版 App（3.5.1）不带 check_id 将被拒绝，须发新版（AA③）；H5 选单 UI 见 AA② | 113 INV-BATCH-001-C / 110 INV-REVERT-001 | 回归 `tests/test_inv_batch_001e_mobile_check_required.py` 6/6 + 重构 `test_inv_batch_001c` 7/7、`test_inv_guard_001` 5/5、`test_inv_revert_001` 8/8 PASSED；`pytest tests/` 1149 passed；涉盘点 verify 全绿；`scripts/verify_wms_bugs.py` PASS。**发布边界**：后端改动，拉取重启 WMS 生效；Android/H5 客户端同步更新见 AA②/AA③ |
...
- **fix3 落地过程（三轮 CI 排障）**：①run `32928804369` 失败——downloadSherpaModel 默认 URL 模型名 `sherpa-onnx-streaming-zh-en-2023-06-26` 在官方 release 不存在（Not Found）；且原 untar 逻辑未处理真实模型包的顶层目录与 epoch/int8 后缀文件名；另发现 SherpaVoiceSttEngine 只从 filesDir 读模型而全 APP 无 assets→filesDir 复制（运行时断链）。修复：URL 改真实存在的 `zipformer-zh-14M-2023-02-23`（70.6MB 纯中文流式 transducer）、解包后按前缀匹配（优先 int8，APK 增量约 15-20MB）平铺重命名为标准四件套、引擎补 `copyModelFromAssets`（BuildConfig.SHERPA_ENABLED/SHERPA_MODEL_DIR 定位）。②run `32930112111` 失败——`implementation("com.k2fsa.sherpaonnx:sherpa-onnx:1.12.13")` 在 Maven Central 不存在（com.k2fsa group 查无，checkDebugAarMetadata 失败）；官方 AAR 只发 GitHub Releases。修复：依赖改 `implementation(files("libs/sherpa-onnx-1.13.6.aar"))` + 新增 `downloadSherpaAar` task（含体积校验拦 302/404）+ CI Verify 加 AAR 校验 + `.gitignore` 排除 `app/libs/*.aar`。③run `32930892537` **success**：APK artifact `app-debug` 98.5MB（含 AAR + int8 模型）。修复提交 `d492ab23e5a7`（断链三层）、`cad67ae2e7f4`（AAR 本地依赖）；回归 sherpa 相关 4 个 verify 文件 51/51 PASSED（新增 T5-T9：真实 URL/平铺重命名/assets 复制/CI 下载校验 AAR/gitignore 排除）。

#### AI-APP-UI-001（已完成）— 安卓原生 App 全界面精致化改版 + 物料档案规格显示修复

- 完成日期：2026-08-28
- 需求原文：手机 app 物料档案规格显示不全；优化手机 APP 所有的界面，要求更加漂亮、更加细腻。
- 提交 SHA（本地，API 通道重放后远程 SHA 以推送验证为准）：`41e6062`（fix BUG-2026-08-28-005）、`b5f06e4`（共享组件库）、`d0c1215`（物料档案两屏）、`bedafae`（首页）、`0726e3e`（登录/个人中心）、`6c13388`（扫码四屏）、`acbb100`（期初库存/仓库弹窗）、`9c45c1d`（AI 三屏）、`da18c7a`（版本号 3.4.0）、台账登记见本提交。
- 修复：BUG-2026-08-28-005 物料档案列表规格 `maxLines = 1` 省略号截断 → 2 行折行（已登记 WMS_BUG_BASELINE，`scripts/verify_wms_bugs.py` 新增 `check_mobile_archive_spec_wrap` 回归 PASS）。注：远程已占用 2026-08-28 的 001-004 编号，本修复取 005。
- 设计体系：新增 `app/android-native-wms/.../ui/components/WmsComponents.kt` 共享组件库（统一顶栏/模块色渐变头部/卡片/空态/主按钮/描边按钮/胶囊徽标/分组标题/信息格/分隔线），`Color.kt` 新增 DividerSoft/BorderSoft/CardAmber；全部 11 个界面（登录、首页、物料档案搜索/详情、扫码入库/出库/盘点/查库存、期初库存、识别单据/识物/识物盘点、个人中心）逐一应用。
- 版本号（BUG-2026-08-16-022 规矩）：versionCode 6→7、versionName 3.3.0→3.4.0，保证新 UI APK 可覆盖安装。
- 专项验证命令及结果：
