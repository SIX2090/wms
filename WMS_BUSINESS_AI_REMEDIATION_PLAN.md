...
>
> **D1/D2 已拍板（2026-09-11，按进销存惯例定案）**——
> D1 = **审批通过后占用**（系统单据均以审批为生效锚点，与采购在途"已审批未入库"口径对称；下单是意向，审批才是承诺）；
> D2 = **新增独立"销售退货入库"单据类型**（退货冲减销售收入与成本，复用采购入库会污染采购统计与财务口径；系统已有 after_sale_out 独立单据先例）。
>
> **P1-6 已完成并推送**（STOCK-TRUTH-P16，原子 1-3：占用派生函数 get_committed_quantities
> + 下单/改单软校验 + 三入口下推硬校验；15 回归含多仓隔离；原子 4 口径写入 INVENTORY_TRUTH.md §2.2）。
...
>
> **F7 已修复并从 CI 豁免清单移除**（2026-09-12）——
> TestBugP21BizReportWarehouseGuard 3 例长期豁免，复核结论是**测试与口径不匹配**，
> 不是实现缺陷：①采购订单本身不记仓库，采购执行表按**来源入库单**的仓库归属
> （`InOrder.warehouse`），而种子只造了采购订单、没造入库单，于是"有数据却查不出"
> 被误读成"守卫误伤"；②"无仓库"的契约写成 200 + 空 data，实现是 400「请选择仓库」——
> 保留实现：返回空会让"你还没选仓库"被渲染成"该仓没有数据"，据此判断"没有采购在途"
> 是危险的。已按三类报表各自的归属口径补齐种子，并加**换仓查询必须为空**的隔离断言
> （多仓串仓是这个系统 20+ 个 BUG 的共同根因）。
>
> **F7 断言真正进 CI 的做法（2026-09-12）**：原断言所在的 verify_bug_P15_P16_P21.py
> 被 ci.yml 的 known_failures **整文件跳过**——"豁免"两年的真实含义是它从没跑过。
> 摘清单要改 workflow 文件，需要带 `workflow` scope 的 token（实测仅 `repo` scope 推
> workflow 文件时 GitHub 伪装成 404 拒绝）。故把守卫断言迁到
> tests/test_report_warehouse_guard.py：test_ 前缀进主 `pytest tests/` 收集，
