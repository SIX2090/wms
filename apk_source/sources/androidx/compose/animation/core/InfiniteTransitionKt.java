...
public final class InfiniteTransitionKt {
    ...
    public static final <T, V extends AnimationVector> State<T> animateValue(final InfiniteTransition $this$animateValue, final T t, final T t2, TwoWayConverter<T, V> twoWayConverter, final InfiniteRepeatableSpec<T> infiniteRepeatableSpec, String label, Composer $composer, int $changed, int i) {
        ...
        if (value$iv$iv == Composer.INSTANCE.getEmpty()) {
            value$iv$iv = $this$animateValue.new TransitionAnimationState(t, t2, twoWayConverter, infiniteRepeatableSpec, label2);
            $composer.updateRememberedValue(value$iv$iv);
        }
        $composer.endReplaceableGroup();
    ...
    public static final State<Float> animateFloat(InfiniteTransition $this$animateFloat, float initialValue, float targetValue, InfiniteRepeatableSpec<Float> infiniteRepeatableSpec, String label, Composer $composer, int $changed, int i) {
        ...
        State<Float> animateValue = animateValue($this$animateFloat, Float.valueOf(initialValue), Float.valueOf(targetValue), VectorConvertersKt.getVectorConverter(FloatCompanionObject.INSTANCE), infiniteRepeatableSpec, label2, $composer, ($changed & 112) | 8 | ($changed & 896) | (($changed << 3) & 57344) | (458752 & ($changed << 3)), 0);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        $composer.endReplaceableGroup();
