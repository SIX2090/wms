...
public final class DragInteractionKt {
    public static final State<Boolean> collectIsDraggedAsState(InteractionSource $this$collectIsDraggedAsState, Composer $composer, int $changed) {
        ...
        boolean invalid$iv = $composer.changed($this$collectIsDraggedAsState) | $composer.changed(isDragged);
        Object it$iv = $composer.rememberedValue();
        if (invalid$iv || it$iv == Composer.INSTANCE.getEmpty()) {
            value$iv = new DragInteractionKt$collectIsDraggedAsState$1$1($this$collectIsDraggedAsState, isDragged, null);
            $composer.updateRememberedValue(value$iv);
        } else {
            value$iv = it$iv;
        }
        $composer.endReplaceableGroup();
        EffectsKt.LaunchedEffect($this$collectIsDraggedAsState, (Function2<? super CoroutineScope, ? super Continuation<? super Unit>, ? extends Object>) value$iv, $composer, ($changed & 14) | 64);
        if (ComposerKt.isTraceInProgress()) {
