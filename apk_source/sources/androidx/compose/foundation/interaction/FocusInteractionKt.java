...
public final class FocusInteractionKt {
    public static final State<Boolean> collectIsFocusedAsState(InteractionSource $this$collectIsFocusedAsState, Composer $composer, int $changed) {
        ...
        boolean invalid$iv = $composer.changed($this$collectIsFocusedAsState) | $composer.changed(isFocused);
        Object it$iv = $composer.rememberedValue();
        if (invalid$iv || it$iv == Composer.INSTANCE.getEmpty()) {
            value$iv = new FocusInteractionKt$collectIsFocusedAsState$1$1($this$collectIsFocusedAsState, isFocused, null);
            $composer.updateRememberedValue(value$iv);
        } else {
            value$iv = it$iv;
        }
        $composer.endReplaceableGroup();
        EffectsKt.LaunchedEffect($this$collectIsFocusedAsState, (Function2<? super CoroutineScope, ? super Continuation<? super Unit>, ? extends Object>) value$iv, $composer, ($changed & 14) | 64);
        if (ComposerKt.isTraceInProgress()) {
