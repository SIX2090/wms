...
final class FillNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo90measure3p2s80s(MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        int minHeight;
        int maxHeight;
        if (Constraints.m6046getHasBoundedWidthimpl(constraints) && this.direction != Direction.Vertical) {
            int width = RangesKt.coerceIn(MathKt.roundToInt(Constraints.m6050getMaxWidthimpl(constraints) * this.fraction), Constraints.m6052getMinWidthimpl(constraints), Constraints.m6050getMaxWidthimpl(constraints));
            minWidth = width;
            maxWidth = width;
        } else {
            minWidth = Constraints.m6052getMinWidthimpl(constraints);
            maxWidth = Constraints.m6050getMaxWidthimpl(constraints);
        }
        if (Constraints.m6045getHasBoundedHeightimpl(constraints) && this.direction != Direction.Horizontal) {
            int height = RangesKt.coerceIn(MathKt.roundToInt(Constraints.m6049getMaxHeightimpl(constraints) * this.fraction), Constraints.m6051getMinHeightimpl(constraints), Constraints.m6049getMaxHeightimpl(constraints));
            minHeight = height;
            maxHeight = height;
        } else {
            minHeight = Constraints.m6051getMinHeightimpl(constraints);
            maxHeight = Constraints.m6049getMaxHeightimpl(constraints);
        }
        final Placeable placeable = measurable.mo5016measureBRTryo0(ConstraintsKt.Constraints(minWidth, maxWidth, minHeight, maxHeight));
