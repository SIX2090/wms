...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo90measure3p2s80s(final MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        int m6052getMinWidthimpl = this.direction != Direction.Vertical ? 0 : Constraints.m6052getMinWidthimpl(constraints);
        int m6051getMinHeightimpl = this.direction == Direction.Horizontal ? Constraints.m6051getMinHeightimpl(constraints) : 0;
        if (this.direction != Direction.Vertical && this.unbounded) {
            m6050getMaxWidthimpl = Integer.MAX_VALUE;
        } else {
            m6050getMaxWidthimpl = Constraints.m6050getMaxWidthimpl(constraints);
        }
        long wrappedConstraints = ConstraintsKt.Constraints(m6052getMinWidthimpl, m6050getMaxWidthimpl, m6051getMinHeightimpl, (this.direction == Direction.Horizontal || !this.unbounded) ? Constraints.m6049getMaxHeightimpl(constraints) : Integer.MAX_VALUE);
        final Placeable placeable = measurable.mo5016measureBRTryo0(wrappedConstraints);
        final int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m6052getMinWidthimpl(constraints), Constraints.m6050getMaxWidthimpl(constraints));
        final int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m6051getMinHeightimpl(constraints), Constraints.m6049getMaxHeightimpl(constraints));
        return MeasureScope.layout$default($this$measure_u2d3p2s80s, wrapperWidth, wrapperHeight, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.WrapContentNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope $this$layout) {
                long position = WrapContentNode.this.getAlignmentCallback().invoke(IntSize.m6256boximpl(IntSizeKt.IntSize(wrapperWidth - placeable.getWidth(), wrapperHeight - placeable.getHeight())), $this$measure_u2d3p2s80s.getLayoutDirection()).getPackedValue();
                Placeable.PlacementScope.m5067place70tqf50$default($this$layout, placeable, position, 0.0f, 2, null);
            }
        }, 4, null);
    }
}
