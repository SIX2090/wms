...
public interface GridCells {
    ...
    public static final class Adaptive implements GridCells {
        ...
        private Adaptive(float minSize) {
            ...
            if (Dp.m6093compareTo0680j_4(this.minSize, Dp.m6094constructorimpl((float) 0)) > 0) {
            } else {
                throw new IllegalArgumentException(("Provided min size " + ((Object) Dp.m6105toStringimpl(this.minSize)) + " should be larger than zero.").toString());
            }
        }
        ...
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            List<Integer> calculateCellsCrossAxisSizeImpl;
            int count = Math.max((availableSize + spacing) / ($this$calculateCrossAxisCellSizes.mo307roundToPx0680j_4(this.minSize) + spacing), 1);
            calculateCellsCrossAxisSizeImpl = LazyGridDslKt.calculateCellsCrossAxisSizeImpl(availableSize, count, spacing);
            return calculateCellsCrossAxisSizeImpl;
        }

        public int hashCode() {
            return Dp.m6100hashCodeimpl(this.minSize);
        }

        public boolean equals(Object other) {
            return (other instanceof Adaptive) && Dp.m6099equalsimpl0(this.minSize, ((Adaptive) other).minSize);
        }
    }
    ...
    public static final class FixedSize implements GridCells {
        ...
        private FixedSize(float size) {
            this.size = size;
            if (Dp.m6093compareTo0680j_4(this.size, Dp.m6094constructorimpl((float) 0)) > 0) {
            } else {
                throw new IllegalArgumentException(("Provided size " + ((Object) Dp.m6105toStringimpl(this.size)) + " should be larger than zero.").toString());
            }
        }

        @Override // androidx.compose.foundation.lazy.grid.GridCells
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            int cellSize = $this$calculateCrossAxisCellSizes.mo307roundToPx0680j_4(this.size);
            int i = 0;
        ...
        public int hashCode() {
            return Dp.m6100hashCodeimpl(this.size);
        }

        public boolean equals(Object other) {
            return (other instanceof FixedSize) && Dp.m6099equalsimpl0(this.size, ((FixedSize) other).size);
        }
    }
}
