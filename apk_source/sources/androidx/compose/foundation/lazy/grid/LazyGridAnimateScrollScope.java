...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public int getVisibleItemScrollOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            return IntOffset.m6223getYimpl(item.getOffset());
        }
        return IntOffset.m6222getXimpl(item.getOffset());
    }
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        m6264getWidthimpl = IntSize.m6263getHeightimpl(visibleItems.get(lineEndIndex).getSize());
                    } else {
                        m6264getWidthimpl = IntSize.m6264getWidthimpl(visibleItems.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, m6264getWidthimpl);
                    lineEndIndex++;
                }
