...
public abstract class LazyGridMeasuredItemProvider {
    ...
    public static /* synthetic */ LazyGridMeasuredItem m703getAndMeasure3p2s80s$default(LazyGridMeasuredItemProvider lazyGridMeasuredItemProvider, int i, int i2, long j, int i3, Object obj) {
        if (obj != null) {
        ...
        return lazyGridMeasuredItemProvider.m704getAndMeasure3p2s80s(i, i2, j);
    }

    /* renamed from: getAndMeasure-3p2s80s, reason: not valid java name */
    public final LazyGridMeasuredItem m704getAndMeasure3p2s80s(int index, int mainAxisSpacing, long constraints) {
        int crossAxisSize;
        Object key = this.itemProvider.getKey(index);
        Object contentType = this.itemProvider.getContentType(index);
        List placeables = this.measureScope.mo726measure0kLqBqw(index, constraints);
        if (Constraints.m6048getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m6052getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m6047getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            crossAxisSize = Constraints.m6051getMinHeightimpl(constraints);
        }
