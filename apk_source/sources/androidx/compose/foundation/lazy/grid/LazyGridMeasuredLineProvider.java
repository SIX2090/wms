...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m705childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span != 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m6060fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m6059fixedHeightOenEA2s(crossAxisSize);
    }

    /* renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m706itemConstraintsOenEA2s(int itemIndex) {
        int span = this.spanLayoutProvider.spanOf(itemIndex, this.spanLayoutProvider.getSlotsPerLine());
        return m705childConstraintsJhjzzOo$foundation_release(0, span);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m683getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).getPackedValue());
            long constraints = m705childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem m704getAndMeasure3p2s80s = this.measuredItemProvider.m704getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i, mainAxisSpacing, constraints);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i] = m704getAndMeasure3p2s80s;
        }
