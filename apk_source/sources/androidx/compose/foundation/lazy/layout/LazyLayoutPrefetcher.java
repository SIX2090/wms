...
public final class LazyLayoutPrefetcher implements RememberObserver, LazyLayoutPrefetchState.Prefetcher, Runnable, Choreographer.FrameCallback {
    ...
    public void run() {
        ...
        Object key;
        Function2 content;
        long latestFrameVsyncNs;
        if (this.prefetchRequests.isEmpty() || !this.prefetchScheduled || !this.isActive) {
            z = false;
        } else {
            if (this.view.getWindowVisibility() == 0) {
                long latestFrameVsyncNs2 = TimeUnit.MILLISECONDS.toNanos(this.view.getDrawingTime());
                long nextFrameNs = latestFrameVsyncNs2 + frameIntervalNs;
                boolean oneOverTimeTaskAllowed = System.nanoTime() > nextFrameNs;
                boolean oneOverTimeTaskAllowed2 = oneOverTimeTaskAllowed;
                ...
                while (this.prefetchRequests.isNotEmpty() && !scheduleForNextFrame) {
                            ...
                            if (request.getPrecomposeHandle() == null) {
                                ...
                                try {
                                    ...
                                    try {
                                        try {
                                            try {
                                                if (!enoughTimeLeft(beforeTimeNs, nextFrameNs, this.averagePrecomposeTimeNs) && !oneOverTimeTaskAllowed2) {
                                                    scheduleForNextFrame = true;
                                                    latestFrameVsyncNs = latestFrameVsyncNs2;
                                                    Unit unit = Unit.INSTANCE;
                                                    Trace.endSection();
                                                    latestFrameVsyncNs2 = latestFrameVsyncNs;
                                                }
                                        ...
                                        oneOverTimeTaskAllowed2 = false;
                                        key = itemProvider.getKey(request.getIndex());
                                        Object contentType = itemProvider.getContentType(request.getIndex());
                                        content = this.itemContentFactory.getContent(request.getIndex(), key, contentType);
                                    } catch (Throwable th3) {
                                        th = th3;
                                    }
                                } catch (Throwable th4) {
                                ...
                                long latestFrameVsyncNs3 = latestFrameVsyncNs2;
                                ...
                                try {
                                    ...
                                    try {
                                        if (!enoughTimeLeft(beforeTimeNs2, nextFrameNs, this.averagePremeasureTimeNs) && !oneOverTimeTaskAllowed2) {
                                            scheduleForNextFrame = true;
                                            Unit unit3 = Unit.INSTANCE;
                                            Trace.endSection();
                                            latestFrameVsyncNs2 = latestFrameVsyncNs3;
                                        }
                                        oneOverTimeTaskAllowed2 = false;
                                        ...
                                        for (int i = 0; i < placeablesCount; i++) {
                                            ...
                                            handle.mo5032premeasure0kLqBqw(placeableIndex, request.getConstraints());
                                        }
                    ...
                    this.prefetchRequests.removeAt(0);
                    latestFrameVsyncNs2 = latestFrameVsyncNs2;
                }
    ...
    public LazyLayoutPrefetchState.PrefetchHandle mo728schedulePrefetch0kLqBqw(int index, long constraints) {
