...
public final class TextDelegateKt {
    ...
    public static final TextDelegate m902updateTextDelegaterm0N8CA(TextDelegate current, AnnotatedString text, TextStyle style, Density density, FontFamily.Resolver fontFamilyResolver, boolean softWrap, int overflow, int maxLines, int minLines, List<AnnotatedString.Range<Placeholder>> list) {
        if (Intrinsics.areEqual(current.getText(), text) && Intrinsics.areEqual(current.getStyle(), style) && current.getSoftWrap() == softWrap && TextOverflow.m6006equalsimpl0(current.getOverflow(), overflow) && current.getMaxLines() == maxLines && current.getMinLines() == minLines && Intrinsics.areEqual(current.getDensity(), density) && Intrinsics.areEqual(current.getPlaceholders(), list) && current.getFontFamilyResolver() == fontFamilyResolver) {
            return current;
        }
