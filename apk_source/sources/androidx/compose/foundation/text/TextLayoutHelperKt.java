...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m935canReuse7_7YC6M(TextLayoutResult $this$canReuse_u2d7_7YC6M, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> list, int maxLines, boolean softWrap, int overflow, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long constraints) {
        TextLayoutInput layoutInput = $this$canReuse_u2d7_7YC6M.getLayoutInput();
        if ($this$canReuse_u2d7_7YC6M.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != maxLines || layoutInput.getSoftWrap() != softWrap || !TextOverflow.m6006equalsimpl0(layoutInput.getOverflow(), overflow) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m6052getMinWidthimpl(constraints) != Constraints.m6052getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (softWrap || TextOverflow.m6006equalsimpl0(overflow, TextOverflow.INSTANCE.m6014getEllipsisgIe3tQ8())) {
            return Constraints.m6050getMaxWidthimpl(constraints) == Constraints.m6050getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m6049getMaxHeightimpl(constraints) == Constraints.m6049getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
