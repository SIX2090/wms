...
final class DefaultElevationOverlay implements ElevationOverlay {
    ...
    public long mo1326apply7g2Lkgo(long color, float elevation, Composer $composer, int $changed) {
        ...
        int $this$dp$iv = Dp.m6093compareTo0680j_4(elevation, Dp.m6094constructorimpl(0));
        if ($this$dp$iv <= 0 || colors.isLight()) {
            foregroundColor = color;
        } else {
            foregroundColor2 = ElevationOverlayKt.m1351calculateForegroundColorCLU3JFs(color, elevation, $composer, ($changed & 14) | ($changed & 112));
            foregroundColor = ColorKt.m3791compositeOverOWjLjI(foregroundColor2, color);
        }
        if (ComposerKt.isTraceInProgress()) {
