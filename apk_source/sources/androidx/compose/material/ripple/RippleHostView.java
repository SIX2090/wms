...
public final class RippleHostView extends View {
    ...
    public final void m1550addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1551updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m3505getXimpl(interaction.getPressPosition()), Offset.m3506getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1551updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m1558setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m3574getWidthimpl(size)), MathKt.roundToInt(Size.m3571getHeightimpl(size)));
