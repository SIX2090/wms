...
public final class AndroidShader_androidKt {
    ...
    public static final Shader m3647ActualLinearGradientShaderVjE6UOU(long from, long to, List<Color> list, List<Float> list2, int tileMode) {
        validateColorStops(list, list2);
        int numTransparentColors = countTransparentColors(list);
        return new android.graphics.LinearGradient(Offset.m3505getXimpl(from), Offset.m3506getYimpl(from), Offset.m3505getXimpl(to), Offset.m3506getYimpl(to), makeTransparentColors(list, numTransparentColors), makeTransparentStops(list2, list, numTransparentColors), AndroidTileMode_androidKt.m3651toAndroidTileMode0vamqd0(tileMode));
    }

    /* renamed from: ActualRadialGradientShader-8uybcMk, reason: not valid java name */
    public static final Shader m3648ActualRadialGradientShader8uybcMk(long center, float radius, List<Color> list, List<Float> list2, int tileMode) {
        validateColorStops(list, list2);
        int numTransparentColors = countTransparentColors(list);
        return new android.graphics.RadialGradient(Offset.m3505getXimpl(center), Offset.m3506getYimpl(center), radius, makeTransparentColors(list, numTransparentColors), makeTransparentStops(list2, list, numTransparentColors), AndroidTileMode_androidKt.m3651toAndroidTileMode0vamqd0(tileMode));
    }

    /* renamed from: ActualSweepGradientShader-9KIMszo, reason: not valid java name */
    public static final Shader m3649ActualSweepGradientShader9KIMszo(long center, List<Color> list, List<Float> list2) {
        validateColorStops(list, list2);
        int numTransparentColors = countTransparentColors(list);
        return new android.graphics.SweepGradient(Offset.m3505getXimpl(center), Offset.m3506getYimpl(center), makeTransparentColors(list, numTransparentColors), makeTransparentStops(list2, list, numTransparentColors));
    }

    /* renamed from: ActualImageShader-F49vj9s, reason: not valid java name */
    public static final Shader m3646ActualImageShaderF49vj9s(ImageBitmap image, int tileModeX, int tileModeY) {
        return new BitmapShader(AndroidImageBitmap_androidKt.asAndroidBitmap(image), AndroidTileMode_androidKt.m3651toAndroidTileMode0vamqd0(tileModeX), AndroidTileMode_androidKt.m3651toAndroidTileMode0vamqd0(tileModeY));
    }
    ...
    public static final int[] makeTransparentColors(List<Color> list, int numTransparentColors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3800toArgb8_81llA(list.get(i).m3756unboximpl());
        }
        return iArr;
    }
    ...
    public static final float[] makeTransparentStops(List<Float> list, List<Color> list2, int numTransparentColors) {
        ...
        for (int i = 1; i < lastIndex; i++) {
            long color = list2.get(i).m3756unboximpl();
            ...
            if (!(Color.m3748getAlphaimpl(color) == 0.0f)) {
