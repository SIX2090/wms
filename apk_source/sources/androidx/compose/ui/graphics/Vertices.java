...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3800toArgb8_81llA(colors.get(i).m3756unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m3506getYimpl = Offset.m3505getXimpl(point);
            } else {
                m3506getYimpl = Offset.m3506getYimpl(point);
            }
            fArr[i] = m3506getYimpl;
        }
        return fArr;
    }
}
