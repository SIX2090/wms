...
public final class MotionEventAdapter {
    ...
    private final long m4820getComposePointerId_I2yYro(int motionEventPointerId) {
        long newId;
        ...
        return PointerId.m4870constructorimpl(newId);
    }
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        ...
        long pointerId = m4820getComposePointerId_I2yYro(motionEventPointerId);
        float pressure = motionEvent.getPressure(index);
        long position2 = OffsetKt.Offset(motionEvent.getX(index), motionEvent.getY(index));
        long originalPositionEventPosition = Offset.m3499copydBAh8RU$default(position2, 0.0f, 0.0f, 3, null);
        if (index == 0) {
            long rawPosition2 = OffsetKt.Offset(motionEvent.getRawX(), motionEvent.getRawY());
            position = positionCalculator.mo4973screenToLocalMKHz9U(rawPosition2);
            rawPosition = rawPosition2;
        } else if (Build.VERSION.SDK_INT >= 29) {
            long rawPosition3 = MotionEventHelper.INSTANCE.m4821toRawOffsetdBAh8RU(motionEvent, index);
            position = positionCalculator.mo4973screenToLocalMKHz9U(rawPosition3);
            rawPosition = rawPosition3;
        } else {
            long rawPosition4 = positionCalculator.mo4972localToScreenMKHz9U(position2);
            position = position2;
            rawPosition = rawPosition4;
        }
        switch (motionEvent.getToolType(index)) {
            case 0:
                toolType = PointerType.INSTANCE.m4970getUnknownT8wyACA();
                break;
            case 1:
                toolType = PointerType.INSTANCE.m4969getTouchT8wyACA();
                break;
            case 2:
                toolType = PointerType.INSTANCE.m4968getStylusT8wyACA();
                break;
            case 3:
                toolType = PointerType.INSTANCE.m4967getMouseT8wyACA();
                break;
            case 4:
                toolType = PointerType.INSTANCE.m4966getEraserT8wyACA();
                break;
            default:
                toolType = PointerType.INSTANCE.m4970getUnknownT8wyACA();
                break;
        ...
        if (motionEvent.getActionMasked() == 8) {
            ...
            scrollDelta = Offset.INSTANCE.m3521getZeroF1C5BW0();
        }
        boolean issuesEnterExit = this.canHover.get(motionEvent.getPointerId(index), false);
        return new PointerInputEventData(pointerId, motionEvent.getEventTime(), rawPosition, position, pressed, pressure, toolType, issuesEnterExit, historical, scrollDelta, originalPositionEventPosition, null);
    }
}
