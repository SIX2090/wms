...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long distance = m5139findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5134getDistanceimpl(distance) < 0.0f && DistanceAndInLayer.m5136isInLayerimpl(distance);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        long bestDistance = m5139findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5130compareToS_HNhKs(bestDistance, distanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m5139findBestHitDistanceptXAw2c() {
        long bestDistance;
        ...
        if (i <= lastIndex) {
            while (true) {
                long distance = DistanceAndInLayer.m5131constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                bestDistance = DistanceAndInLayer.m5130compareToS_HNhKs(distance, bestDistance) < 0 ? distance : bestDistance;
                if (DistanceAndInLayer.m5134getDistanceimpl(bestDistance) >= 0.0f || !DistanceAndInLayer.m5136isInLayerimpl(bestDistance)) {
                    if (i == lastIndex) {
                        break;
                    }
                    i++;
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        long previousDistance = m5139findBestHitDistanceptXAw2c();
        int previousHitDepth = this.hitDepth;
        this.hitDepth = CollectionsKt.getLastIndex(this);
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(this) && DistanceAndInLayer.m5130compareToS_HNhKs(previousDistance, m5139findBestHitDistanceptXAw2c()) > 0) {
