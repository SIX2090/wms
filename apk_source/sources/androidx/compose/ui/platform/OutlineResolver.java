...
public final class OutlineResolver {
    ...
    public OutlineResolver(Density density) {
        ...
        this.size = Size.INSTANCE.m3583getZeroNHjbRc();
        this.shape = RectangleShapeKt.getRectangleShape();
        this.rectTopLeft = Offset.INSTANCE.m3521getZeroF1C5BW0();
        this.rectSize = Size.INSTANCE.m3583getZeroNHjbRc();
    ...
    public final boolean m5371isInOutlinek4lQ0M(long position) {
        androidx.compose.ui.graphics.Outline outline;
        if (this.outlineNeeded && (outline = this.calculatedOutline) != null) {
            return ShapeContainingUtilKt.isInOutline(outline, Offset.m3505getXimpl(position), Offset.m3506getYimpl(position), this.tmpTouchPointPath, this.tmpOpPath);
        }
        return true;
    }
    ...
    public final void clipToOutline(Canvas canvas) {
        ...
        if (targetPath != null) {
            Canvas.m3717clipPathmtrdDE$default(canvas, targetPath, 0, 2, null);
            return;
        }
        if (this.roundedCornerRadius > 0.0f) {
            Path roundRectClipPath = this.tmpPath;
            RoundRect roundRect = this.tmpRoundRect;
            if (roundRectClipPath == null || !m5370isSameBounds4L21HEs(roundRect, this.rectTopLeft, this.rectSize, this.roundedCornerRadius)) {
                RoundRect roundRect2 = RoundRectKt.m3559RoundRectgG7oq9Y(Offset.m3505getXimpl(this.rectTopLeft), Offset.m3506getYimpl(this.rectTopLeft), Offset.m3505getXimpl(this.rectTopLeft) + Size.m3574getWidthimpl(this.rectSize), Offset.m3506getYimpl(this.rectTopLeft) + Size.m3571getHeightimpl(this.rectSize), CornerRadiusKt.CornerRadius$default(this.roundedCornerRadius, 0.0f, 2, null));
            ...
            Canvas.m3717clipPathmtrdDE$default(canvas, roundRectClipPath, 0, 2, null);
            return;
        }
        Canvas.m3718clipRectN_I0leg$default(canvas, Offset.m3505getXimpl(this.rectTopLeft), Offset.m3506getYimpl(this.rectTopLeft), Offset.m3505getXimpl(this.rectTopLeft) + Size.m3574getWidthimpl(this.rectSize), Offset.m3506getYimpl(this.rectTopLeft) + Size.m3571getHeightimpl(this.rectSize), 0, 16, null);
    }

    /* renamed from: update-uvyYCjk, reason: not valid java name */
    public final void m5372updateuvyYCjk(long size) {
        if (!Size.m3570equalsimpl0(this.size, size)) {
            this.size = size;
            this.cacheIsDirty = true;
        }
    }
    ...
    private final void updateCache() {
        if (this.cacheIsDirty) {
            ...
            if (this.outlineNeeded && Size.m3574getWidthimpl(this.size) > 0.0f && Size.m3571getHeightimpl(this.size) > 0.0f) {
    ...
    private final void updateCacheWithRoundRect(RoundRect roundRect) {
        float radius = CornerRadius.m3480getXimpl(roundRect.m3555getTopLeftCornerRadiuskKHJgLs());
    ...
    private final boolean m5370isSameBounds4L21HEs(RoundRect $this$isSameBounds_u2d4L21HEs, long offset, long size, float radius) {
        if ($this$isSameBounds_u2d4L21HEs == null || !RoundRectKt.isSimple($this$isSameBounds_u2d4L21HEs)) {
            return false;
        }
        if ($this$isSameBounds_u2d4L21HEs.getLeft() == Offset.m3505getXimpl(offset)) {
            if ($this$isSameBounds_u2d4L21HEs.getTop() == Offset.m3506getYimpl(offset)) {
                if ($this$isSameBounds_u2d4L21HEs.getRight() == Offset.m3505getXimpl(offset) + Size.m3574getWidthimpl(size)) {
                    if ($this$isSameBounds_u2d4L21HEs.getBottom() == Offset.m3506getYimpl(offset) + Size.m3571getHeightimpl(size)) {
                        return (CornerRadius.m3480getXimpl($this$isSameBounds_u2d4L21HEs.m3555getTopLeftCornerRadiuskKHJgLs()) > radius ? 1 : (CornerRadius.m3480getXimpl($this$isSameBounds_u2d4L21HEs.m3555getTopLeftCornerRadiuskKHJgLs()) == radius ? 0 : -1)) == 0;
                    }
                    return false;
                }
                return false;
