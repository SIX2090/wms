    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m3494boximpl(OffsetKt.Offset(Math.abs(Offset.m3505getXimpl(el1.getBoundsInRoot().m3535getCenterF1C5BW0()) - Offset.m3505getXimpl(el2.getBoundsInRoot().m3535getCenterF1C5BW0())), Math.abs(Offset.m3506getYimpl(el1.getBoundsInRoot().m3535getCenterF1C5BW0()) - Offset.m3506getYimpl(el2.getBoundsInRoot().m3535getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        ...
        if ($this$fastZipWithNext$iv.size() == 1) {
            ...
            if (1 <= lastIndex2) {
                while (true) {
                    ...
                    accumulator$iv = Offset.m3494boximpl(Offset.m3510plusMKHz9U(result, element));
                    if (i$iv2 == lastIndex2) {
                        break;
                    }
                    i$iv2++;
        ...
        float deltaX = Offset.m3495component1impl(packedValue);
        float deltaY = Offset.m3496component2impl(packedValue);
