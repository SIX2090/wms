...
public final class TextMeasurer {
    ...
    public static /* synthetic */ TextLayoutResult m5547measurexDpz5zY$default(TextMeasurer textMeasurer, AnnotatedString annotatedString, TextStyle textStyle, int i, boolean z, int i2, List list, long j, LayoutDirection layoutDirection, Density density, FontFamily.Resolver resolver, boolean z2, int i3, Object obj) {
        return textMeasurer.m5549measurexDpz5zY(annotatedString, (i3 & 2) != 0 ? TextStyle.INSTANCE.getDefault() : textStyle, (i3 & 4) != 0 ? TextOverflow.INSTANCE.m6013getClipgIe3tQ8() : i, (i3 & 8) != 0 ? true : z, (i3 & 16) != 0 ? Integer.MAX_VALUE : i2, (i3 & 32) != 0 ? CollectionsKt.emptyList() : list, (i3 & 64) != 0 ? ConstraintsKt.Constraints$default(0, 0, 0, 0, 15, null) : j, (i3 & 128) != 0 ? textMeasurer.defaultLayoutDirection : layoutDirection, (i3 & 256) != 0 ? textMeasurer.defaultDensity : density, (i3 & 512) != 0 ? textMeasurer.defaultFontFamilyResolver : resolver, (i3 & 1024) != 0 ? false : z2);
    }

    /* renamed from: measure-xDpz5zY, reason: not valid java name */
    public final TextLayoutResult m5549measurexDpz5zY(AnnotatedString text, TextStyle style, int overflow, boolean softWrap, int maxLines, List<AnnotatedString.Range<Placeholder>> placeholders, long constraints, LayoutDirection layoutDirection, Density density, FontFamily.Resolver fontFamilyResolver, boolean skipCache) {
        ...
        if (cacheResult != null) {
            return cacheResult.m5542copyO0kMr_c(requestedTextLayoutInput, ConstraintsKt.m6061constrain4WqzIAM(constraints, IntSizeKt.IntSize(ParagraphKt.ceilToInt(cacheResult.getMultiParagraph().getWidth()), ParagraphKt.ceilToInt(cacheResult.getMultiParagraph().getHeight()))));
        }
    ...
    public final TextLayoutResult m5548measurewNUYSr0(String text, TextStyle style, int overflow, boolean softWrap, int maxLines, long constraints, LayoutDirection layoutDirection, Density density, FontFamily.Resolver fontFamilyResolver, boolean skipCache) {
        return m5547measurexDpz5zY$default(this, new AnnotatedString(text, null, null, 6, null), style, overflow, softWrap, maxLines, null, constraints, layoutDirection, density, fontFamilyResolver, skipCache, 32, null);
    }
    ...
    public static final class Companion {
        ...
        public final TextLayoutResult layout(TextLayoutInput textLayoutInput) {
            ...
            int minWidth = Constraints.m6052getMinWidthimpl(textLayoutInput.getConstraints());
            boolean overwriteMaxLines = false;
            boolean widthMatters = textLayoutInput.getSoftWrap() || TextOverflow.m6006equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m6014getEllipsisgIe3tQ8());
            if (widthMatters && Constraints.m6046getHasBoundedWidthimpl(textLayoutInput.getConstraints())) {
                maxWidth = Constraints.m6050getMaxWidthimpl(textLayoutInput.getConstraints());
            } else {
                maxWidth = Integer.MAX_VALUE;
            }
            if (!textLayoutInput.getSoftWrap() && TextOverflow.m6006equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m6014getEllipsisgIe3tQ8())) {
            ...
            MultiParagraph multiParagraph = new MultiParagraph(nonNullIntrinsics, ConstraintsKt.Constraints$default(0, width, 0, Constraints.m6049getMaxHeightimpl(textLayoutInput.getConstraints()), 5, null), finalMaxLines, TextOverflow.m6006equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m6014getEllipsisgIe3tQ8()), null);
            return new TextLayoutResult(textLayoutInput, multiParagraph, ConstraintsKt.m6061constrain4WqzIAM(textLayoutInput.getConstraints(), IntSizeKt.IntSize((int) Math.ceil(multiParagraph.getWidth()), (int) Math.ceil(multiParagraph.getHeight()))), null);
        }
    }
}
