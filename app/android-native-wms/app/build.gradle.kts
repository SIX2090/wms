...
android {
    ...
    defaultConfig {
        ...
        // 3.8.6(20)=BUG-2026-09-18-006 出库/入库页提交按钮被顶部内容顶出屏幕（两处候选列表限高 + 顶部区可滚）；
        //           BUG-2026-09-18-007 扫码页标题去掉写死的"扫码"（与手动添加/语音建单路径对齐）。
        // 3.8.7(21)=BUG-2026-09-18-008 手机端入库单头字段补全（供应商下拉 + 备注输入，写入 InOrder.supplier_id/remark）。
        // 3.8.8(22)=BUG-2026-09-18-009 扫码页基类改用显式布尔参数驱动库位/取证功能，不再比对按钮文案。
        // 3.8.9(23)=BUG-2026-09-18-010 入库确认弹窗补齐收货仓库/供应商/合同/备注，并加"已选仓库"前置校验。
        // 3.9.0(24)=BUG-2026-09-18-011 出库提交按钮文案统一为「确认出库」（原先按钮「提交出库」与弹窗「确认出库」两套说法并存）。
        // 3.9.1(25)=AI-MOB-RPT-F01 收尾：出入库明细 Android 消费页（日期范围 + 按仓流水，只读）。
