...
def auto_migrate_database():
    ...
    try:
        ...
        # 迁移失败必须记录日志，避免管理员无法感知迁移异常导致后续运行时才暴露问题
        try:
...
if not app.config.get('SECRET_KEY'):
    ...
    _secret_key_file = os.path.join(os.path.dirname(__file__), 'instance', 'secret_key')
    _persisted_key = None
    try:
        with open(_secret_key_file, 'r', encoding='utf-8') as _sk_f:
            _persisted_key = _sk_f.read().strip()
    except OSError:
        _persisted_key = None
...
if not app.config.get('WECHAT_HELPER_TOKEN'):
    _wh_token_file = os.path.join(os.path.dirname(__file__), 'instance', 'wechat_helper_token')
    _persisted_wh_token = None
    try:
        with open(_wh_token_file, 'r', encoding='utf-8') as _wh_f:
            _persisted_wh_token = _wh_f.read().strip()
    except OSError:
        _persisted_wh_token = None
    if _persisted_wh_token and len(_persisted_wh_token) >= 16:
        ...
        try:
            os.makedirs(os.path.dirname(_wh_token_file), exist_ok=True)
            with open(_wh_token_file, 'w', encoding='utf-8') as _wh_f:
                _wh_f.write(_new_wh_token)
            try:
                os.chmod(_wh_token_file, 0o600)
            except OSError:
                pass
        except OSError as _wh_err:
            app.logger.error(f'无法持久化 WECHAT_HELPER_TOKEN: {_wh_err}')
        app.logger.warning('WECHAT_HELPER_TOKEN not set. A random token was generated and persisted to instance/wechat_helper_token.')
...
def backup_sqlite_on_startup():
    ...
    if not uri.startswith(prefix):
        return
    db_path = Path(uri[len(prefix):]).resolve()
    if not db_path.exists() or db_path.stat().st_size == 0:
        return
    backup_dir = Path(app.config.get('BACKUP_FOLDER') or Path(__file__).resolve().parent / 'backups')
    backup_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    backup_path = backup_dir / f'auto_startup_{timestamp}.db'
    try:
        with sqlite3.connect(str(db_path)) as src_conn, sqlite3.connect(str(backup_path)) as dst_conn:
            src_conn.backup(dst_conn)
        backups = sorted(backup_dir.glob('auto_startup_*.db'), key=lambda path: path.stat().st_mtime, reverse=True)
        for old_backup in backups[10:]:
            try:
                old_backup.unlink()
            except OSError:
                app.logger.warning('Failed to remove old startup backup: %s', old_backup)
...
app.jinja_env.filters['currency_cn'] = currency_cn
...
SYSTEM_SETTING_GROUPS = [
    {
        ...
        'title': '基础资料',
        'icon': 'bi-database',
        'description': '控制物料、往来单位等基础档案的维护规则。',
        'settings': [
            {
                'key': 'material_code_editable',
                'label': '物料编码可修改',
                'type': 'bool',
                'default': '1',
                'remark': '关闭后，已创建物料的物料编码不允许在编辑时修改。',
            },
            {
                'key': 'business_partner_duplicate_name_blocked',
                'label': '客户/供应商名称禁止重复',
                'type': 'bool',
                'default': '1',
                'remark': '用于控制往来单位基础资料的名称重复策略；当前客户、供应商名称已按唯一规则管理。',
            },
        ],
    },
    {
        ...
        'settings': [
            {
                ...
                'type': 'bool',
                'default': '0',
                'remark': '关闭后，出库、售后出库、发料等扣减库存时必须校验总库存充足。',
            },
            {
                'key': 'prefer_default_warehouse',
                'label': '录单优先取默认仓库',
                'type': 'bool',
                'default': '1',
                'remark': '用于后续单据新增时自动带出默认仓库，减少重复选择。',
            },
            {
                'key': 'warehouse_stock_threshold_enabled',
                'label': '按仓库设置最高/最低/安全库存',
                'type': 'bool',
                'default': '0',
                'remark': '开启后可扩展为按仓库维护库存上下限；当前物料预警仍按物料主档库存上下限执行。',
            },
            {
                'key': 'stock_threshold_control_mode',
                'label': '最低最高库存控制',
                'type': 'select',
                'default': 'prompt',
                'options': [
                ...
                'remark': '库存低于最低或高于最高时的控制方式。建议先用提示，稳定后再改为禁止。',
            },
            {
                'key': 'location_management_enabled',
                'label': '启用库位管理',
                'type': 'bool',
                'default': '0',
                'remark': '开启后，库存查询显示库位明细，出入库会同步仓库/库位库存，出库会校验所选仓库库存。',
            },
            {
                'key': 'inventory_alert_enabled',
                'label': '启用库存预警',
                'type': 'bool',
                'default': '1' if app.config.get('INVENTORY_ALERT_ENABLED', False) else '0',
                'remark': '开启后显示库存预警入口，并启用最低库存、预警状态和低库存统计。',
            },
            {
                'key': 'allow_negative_location_stock',
                'label': '允许货位负库存',
                'type': 'bool',
                'default': '0',
                'remark': '关闭后，启用库位管理时出库必须校验对应货位库存充足。',
            },
            {
                'key': 'location_available_stock_control',
                'label': '控制货位可用量',
                'type': 'bool',
                'default': '1',
                'remark': '开启后按货位库存控制出库数量，避免同一货位超发。',
            },
            {
                'key': 'location_auto_match_enabled',
                'label': '自动匹配货位',
                'type': 'bool',
                'default': '1',
                'remark': '开启后可按匹配规则自动推荐或带出货位。',
            },
            {
                'key': 'location_auto_pick_mode',
                'label': '自动带出货位方式',
                'type': 'select',
                'default': 'by_available',
                'options': [
                    {'value': 'default', 'label': '自动带出默认货位'},
                    {'value': 'by_available', 'label': '按货位可用量自动带出'},
                    {'value': 'by_existing_stock', 'label': '按货位现存量自动带出'},
                ],
                'remark': '用于扫码或出库录入时的货位推荐方式。',
            },
            {
                'key': 'location_match_rules',
                'label': '货位匹配规则',
                'type': 'select',
                'default': 'priority_then_small',
                'options': [
                    {'value': 'priority_then_small', 'label': '优先货位，再按数量由小到大'},
                    {'value': 'small_to_large', 'label': '按货位数量由小到大'},
                    {'value': 'large_to_small', 'label': '按货位数量由大到小'},
                ],
                'remark': '一个物料存在多个货位时，自动匹配货位的排序规则。',
            },
            {
                'key': 'split_detail_by_location',
                'label': '按货位拆分明细行',
                'type': 'bool',
                'default': '1',
                'remark': '开启后，一个物料从多个货位出库时应拆分明细，方便核对货位库存。',
            },
            {
                'key': 'location_required_on_save',
                'label': '库存单据保存时货位必填',
                'type': 'bool',
                'default': '1',
                'remark': '启用库位管理后，库存单据保存或审核时要求填写货位/仓库信息。',
            },
        ],
    },
    {
        ...
        'title': '采购参数',
        'icon': 'bi-cart-check',
        'description': '控制采购入库与采购订单的来源、数量和重复明细规则。',
        'settings': [
            {
            ...
            },
            {
                ...
                'type': 'bool',
                'default': '1',
                'remark': '为后续采购退货流程预留；采购退货必须关联原采购订单或采购入库来源。',
            },
            {
                'key': 'purchase_receipt_strict_order',
                'label': '按照采购订单严格入库',
                'type': 'bool',
                'default': '1',
                'remark': '开启后，采购入库数量按采购订单未入库数量控制。',
            },
            {
                'key': 'purchase_over_receive_control_mode',
                'label': '超采购订单数量控制方式',
                'type': 'select',
                'default': 'forbid',
                'options': [
            ...
            },
            {
                ...
                'type': 'bool',
                'default': '1',
                'remark': '关闭后，采购订单不允许下推或选单生成采购入库单。',
            },
        ],
    },
    {
        ...
        'title': '条码管理',
        'icon': 'bi-upc-scan',
        'description': '控制扫码、标签和条码录入相关行为。',
        'settings': [
            {
                'key': 'barcode_management_enabled',
                'label': '启用条码管理',
                'type': 'bool',
                'default': '1',
                'remark': '开启后保留标签条码打印和扫码业务入口；当前系统默认以物料编码作为条码。',
            },
            {
                'key': 'barcode_one_code_multi_material',
                'label': '允许一码多物',
                'type': 'bool',
                'default': '0',
                'remark': '默认关闭。关闭时应保持一个条码只对应一个物料，避免扫码出入库选错物料。',
            },
            {
                'key': 'barcode_merge_same_detail',
                'label': '扫码相同明细自动合并',
                'type': 'bool',
                'default': '1',
                'remark': '开启后，连续扫码录入相同物料时可合并到同一明细行，减少重复行。',
            },
            {
                'key': 'barcode_regenerate_after_document_change',
                'label': '修改单据后重新生成条码',
                'type': 'bool',
                'default': '0',
                'remark': '用于后续单据条码和标签重打控制，避免条码内容与单据内容不一致。',
            },
        ],
    },
    {
        ...
        'description': '控制入库、出库、盘点、调拨等单据的通用行为。',
        'settings': [
            {
                'key': 'approval_flow_enabled',
                'label': '启用审核功能',
                'type': 'bool',
                'default': '0',
                'remark': '默认关闭。开启后才启用单据审核、反审等流程控制。',
            },
            {
                'key': 'document_tabs_enabled',
                'label': '单据多页签',
                'type': 'bool',
                'default': '1',
                'remark': '开启后允许在系统内多页签切换单据和基础资料页面。',
            },
            {
                'key': 'document_detail_column_sort_enabled',
                'label': '单据明细列排序',
                'type': 'bool',
                'default': '1',
                'remark': '开启后允许拖动单据明细字段位置并保存个人列顺序。',
            },
            {
                'key': 'document_column_customization_without_permission',
                'label': '无单据设置权限也可调整列宽',
                'type': 'bool',
                'default': '1',
                'remark': '开启后普通做单用户也能调整明细列宽、字段位置，提高录单效率。',
            },
            {
                'key': 'document_change_reason_required',
                'label': '变更时记录变更原因',
                'type': 'bool',
                'default': '1',
                'remark': '用于后续对反审、删除、修改库存等高风险操作要求填写原因。',
            },
            {
                'key': 'document_delete_detail_log_enabled',
                'label': '删除单据记录详细日志',
                'type': 'bool',
                'default': '1',
                'remark': '开启后删除单据、删除明细等操作应记录操作者、对象和时间。',
            },
            {
                'key': 'document_multi_template_enabled',
                'label': '单据多模板',
                'type': 'bool',
                'default': '1',
                'remark': '开启后入库单、出库单、标签等允许维护多套打印模板。',
            },
        ],
    },
    {
        ...
        'description': '控制 Excel 导入导出的数据量上限。',
        'settings': [
            {
                'key': 'import_max_rows',
                'label': '导入最大数量',
                ...
                'max': 50000,
                'unit': '行',
                'remark': '单次 Excel 导入允许处理的最大数据行数，防止大文件拖慢系统。',
            },
            {
                'key': 'export_max_rows',
                'label': '导出最大数量',
                ...
                'max': 100000,
                'unit': '行',
                'remark': '单次列表导出允许输出的最大数据行数，超过后请先缩小筛选范围。',
            },
        ],
    },
    {
        ...
        'title': '打印参数',
        'icon': 'bi-printer',
        'description': '控制单据、列表和标签打印方式。',
        'settings': [
            {
                'key': 'print_template_only',
                'label': '打印时只显示设计器模板',
                'type': 'bool',
                'default': '0',
                'remark': '开启后打印入口优先使用系统维护的模板，减少临时格式差异。',
            },
            {
                'key': 'list_print_merge_rows',
                'label': '列表打印合并行',
                'type': 'bool',
                'default': '0',
        ...
        'settings': [
            {
                ...
                'label': '启用大模型理解',
                'type': 'bool',
                'default': '1' if app.config.get('WMS_LLM_ENABLED', True) else '0',
                'remark': '开启后，AI助手会先调用大模型理解用户意图；关闭或调用失败时仍使用本地规则助手。',
            },
            {
                'key': 'ai_llm_base_url',
                'label': '接口地址',
                'type': 'text',
                'default': app.config.get('WMS_LLM_BASE_URL', 'https://api.openai.com/v1'),
                'wide': True,
                'remark': '填写 OpenAI 兼容接口地址。可填 https://api.openai.com/v1 或完整 /v1/chat/completions。',
            },
            {
                'key': 'ai_llm_model',
                'label': '模型名称',
                'type': 'text',
                'default': app.config.get('WMS_LLM_MODEL', 'gpt-4.1-mini'),
            ...
            },
            {
                ...
                'type': 'bool',
                'default': '1',
                'remark': '开启后，AI浮窗上传或粘贴图片时会把图片发送给大模型分析；必须使用支持视觉的 OpenAI 兼容模型。关闭后只保留文字问答。',
            },
            {
                'key': 'ai_llm_api_key',
                'label': 'API Key',
                'type': 'secret',
                'default': '',
                'config_key': 'WMS_LLM_API_KEY',
                'wide': True,
                'remark': '保存后不会在页面明文显示。留空保存表示保留原 Key；首次配置必须填写。',
            },
            {
                ...
                'max': 60,
                'unit': '秒',
                'remark': '大模型请求超过该时间会自动回退本地规则助手，避免影响仓库系统操作。',
            },
            {
    ...
    },
    {
        ...
        'title': '数据精度',
        'icon': 'bi-123',
        'description': '统一数量、单价、金额等小数位显示和计算基础。',
        'settings': [
            {
            ...
            },
            {
                ...
                'max': 8,
                'unit': '位',
                'remark': '用于采购价、出入库单价等单价字段精度控制。',
            },
            {
                'key': 'amount_decimal_places',
                'label': '金额小数位',
                'type': 'int',
                'default': '2',
                'min': 0,
    ...
    },
    {
        ...
        'title': '登录安全',
        'icon': 'bi-shield-lock',
        'description': '控制账号登录失败后的锁定策略。',
        'settings': [
            {
                'key': 'max_login_failures',
                'label': '最大登录失败次数',
                'type': 'int',
                'default': str(MAX_LOGIN_FAILURES),
                'min': 1,
                'max': 20,
                'unit': '次',
                'remark': '同一账号连续输错达到该次数后自动锁定。',
            },
            {
                'key': 'account_lock_minutes',
                'label': '账号锁定时长',
                'type': 'int',
                'default': str(ACCOUNT_LOCK_MINUTES),
                'min': 1,
                'max': 1440,
                'unit': '分钟',
                'remark': '账号被锁定后，需要等待该时长才能再次尝试登录。',
            },
        ],
    },
]
...
def set_system_setting(key, value):
    try:
        ...
        db.session.rollback()
        if 'system_setting' in str(exc):
            raise RuntimeError('系统设置表不存在；当前为不改数据库更新模式，不能保存该设置') from exc
        raise
    if not setting:
...
def get_system_setting_int(key, default=0):
    try:
        value = int(get_system_setting(key, str(default)))
    except (TypeError, ValueError):
        return default
    definition = SYSTEM_SETTING_DEFINITIONS.get(key) or {}
...
def get_grouped_system_settings():
    ...
    for group in SYSTEM_SETTING_GROUPS:
        ...
        for definition in group['settings']:
            key = definition['key']
            raw_value = get_system_setting(key, definition.get('default', ''))
            item = dict(definition)
            item['value'] = raw_value
            if item.get('type') == 'bool':
                item['checked'] = raw_value == '1'
            elif item.get('type') == 'secret':
                env_key = definition.get('config_key')
                item['has_value'] = bool(raw_value or (env_key and app.config.get(env_key)))
                item['value'] = ''
            items.append(item)
        grouped.append({
            'key': group['key'],
            'title': group['title'],
            'icon': group.get('icon', 'bi-gear'),
...
def handle_exception(e):
    ...
    if isinstance(e, HTTPException):
        if wants_json_error_response():
            return jsonify({'status': 'error', 'msg': e.description or e.name}), e.code
        return e
...
# ==================== 权限检查装饰器 ====================
# ==================== Document number ====================
def generate_order_no(prefix='NO'):
    """Generate order number with database lock to prevent duplicates in concurrent environment."""
    if prefix == 'OUT':
        prefix = 'OU'
    ...
    for attempt in range(max_retries):
        try:
            if prefix in ('IN', 'PI'):
                ...
                last_order = InventoryCheckScan.query.filter(InventoryCheckScan.check_no.like(f'{prefix}{year_month}%')).order_by(InventoryCheckScan.id.desc()).with_for_update().first()
            elif prefix == 'SC':
                last_order = SubcontractOrder.query.filter(SubcontractOrder.order_no.like(f'{prefix}{year_month}%')).order_by(SubcontractOrder.id.desc()).with_for_update().first()
            elif prefix == 'SF':
                last_order = SubcontractIssue.query.filter(SubcontractIssue.issue_no.like(f'{prefix}{year_month}%')).order_by(SubcontractIssue.id.desc()).with_for_update().first()
            elif prefix == 'SR':
                last_order = SubcontractReceive.query.filter(SubcontractReceive.receive_no.like(f'{prefix}{year_month}%')).order_by(SubcontractReceive.id.desc()).with_for_update().first()
            elif prefix == 'PR':
                last_order = PurchaseRequest.query.filter(PurchaseRequest.request_no.like(f'{prefix}{year_month}%')).order_by(PurchaseRequest.id.desc()).with_for_update().first()
            elif prefix == 'PO':
                last_order = PurchaseOrder.query.filter(PurchaseOrder.order_no.like(f'{prefix}{year_month}%')).order_by(PurchaseOrder.id.desc()).with_for_update().first()
            elif prefix == 'ASO':
                last_order = AfterSaleOutOrder.query.filter(AfterSaleOutOrder.order_no.like(f'{prefix}{year_month}%')).order_by(AfterSaleOutOrder.id.desc()).with_for_update().first()
            elif prefix == 'BOM':
                last_order = BOM.query.filter(BOM.bom_no.like(f'{prefix}{year_month}%')).order_by(BOM.id.desc()).with_for_update().first()
            elif prefix == 'REQ':
                last_order = ProductionRequisition.query.filter(ProductionRequisition.req_no.like(f'{prefix}{year_month}%')).order_by(ProductionRequisition.id.desc()).with_for_update().first()
            elif prefix == 'TF':
            ...
            if last_order:
                if prefix in ('IN', 'PI', 'OU', 'SO', 'SE', 'PO'):
                    last_no = last_order.order_no
                elif prefix in ('CK', 'SC'):
                    last_no = last_order.check_no
                elif prefix == 'CS':
                    last_no = last_order.check_no
                elif prefix == 'SF':
                    last_no = last_order.issue_no
                elif prefix == 'SR':
                    last_no = last_order.receive_no
                elif prefix == 'PR':
                    last_no = last_order.request_no
                elif prefix == 'ASO':
                    last_no = last_order.order_no
                elif prefix == 'BOM':
                    last_no = last_order.bom_no
                elif prefix == 'REQ':
                    last_no = last_order.req_no
                elif prefix == 'TF':
...
def deduct_stock_atomic(material_id, quantity, transaction_type=None, reference_type=None, reference_id=None, remark=None):
    ...
    调用方应 rollback 并返回业务失败。
    不改动 material 实例的 Python 属性；扣减完后由上层按需 refresh/读回。
    """
    qty = normalize_stock_quantity(quantity or 0)
    if qty <= 0:
        return False, '扣减数量必须大于 0', None
    ...
    if not rowcount:
        current = normalize_stock_quantity(mat.stock or 0)
        return False, f'物料 {mat.code} 库存不足或并发冲突，当前库存：{current:.2f}', mat

    # 让 ORM 实例和 DB 状态一致，避免 session 里旧 stock 污染后续逻辑
...
def log_audit(operation, target_type, target_id, target_name=None, old_data=None, new_data=None, status='success', reason=None):
    """Write an operation audit record."""
    try:
...
def ensure_default_warehouses():
    ...
    for item in DEFAULT_WAREHOUSES:
        if Warehouse.query.filter_by(name=item['name']).first():
            continue
...
DEFAULT_LABEL_CELLS = [
    ...
    {'row': 2, 'col': 0, 'field': 'code', 'style': {'fontSize': 10, 'textAlign': 'center', 'color': '#111827'}},
...
DEFAULT_LABEL_LAYOUT = {
    ...
    'rowHeights': [24, 36, 22, 22, 22],
}
for cell in DEFAULT_LABEL_CELLS:
    DEFAULT_LABEL_LAYOUT[f"{cell['row']}-{cell['col']}"] = {
        'field': cell['field'],
        'label': {
            'name': '物料名称',
            'barcode': '条码',
...
def _normalize_label_template_layout(layout):
    ...
    for key, cell in layout.items():
        if not isinstance(key, str) or '-' not in key or not isinstance(cell, dict):
            continue
        ...
        keyed_cells.append({
            ...
            'col': int(col_text),
            'field': field,
            'label': cell.get('label') or field,
            'style': cell.get('style') or {},
            'barcodeWidth': cell.get('barcodeWidth'),
            'barcodeHeight': cell.get('barcodeHeight'),
...
def purchase_request_item_has_material(item):
    return bool(
        ...
        (getattr(item, 'material_code', None) or '').strip() or
...
def update_purchase_order_status(order):
    ...
    if received_qty <= 0:
        order.status = 'pending'
    elif received_qty + STOCK_COMPARE_EPSILON >= total_qty:
...
def validate_purchase_in_order_source(order):
    ...
    if not items:
        return True, None
    if any(not item.source_purchase_order_item_id for item in items):
...
def validate_purchase_receive_quantity(source_item, receive_qty, material_code=None):
    ...
    if receive_qty - remain_qty > STOCK_COMPARE_EPSILON:
        code = material_code or (source_item.material.code if source_item.material else source_item.material_id)
...
def build_purchase_request_execution(valid_items):
    ...
    if item_ids:
        rows = db.session.query(
            ...
            PurchaseOrderItem.purchase_request_item_id.in_(item_ids),
            PurchaseOrder.status != 'closed',
        ).group_by(PurchaseOrderItem.purchase_request_item_id).all()
    ...
    for item in valid_items:
        request_qty = round_to_2_decimals(item.quantity or 0)
        ordered_qty = round_to_2_decimals(ordered_by_item.get(item.id, 0))
        received_qty = round_to_2_decimals(received_by_item.get(item.id, 0))
        remaining_to_order = round_to_2_decimals(max(request_qty - ordered_qty, 0))
        remaining_to_receive = round_to_2_decimals(max(ordered_qty - received_qty, 0))
        if received_qty + STOCK_COMPARE_EPSILON >= request_qty and request_qty > 0:
            status = 'completed'
...
def build_purchase_order_execution(order):
    ...
    completed_by_item = {item_id: 0 for item_id in item_ids}
    if item_ids:
        rows = db.session.query(
        ...
        for item_id, completed_qty in rows:
            completed_by_item[item_id] = round_to_2_decimals(completed_qty or 0)

    execution = {}
    for item in items:
        order_qty = round_to_2_decimals(item.quantity or 0)
        pushed_qty = round_to_2_decimals(item.received_quantity or 0)
        completed_qty = round_to_2_decimals(completed_by_item.get(item.id, 0))
        pending_receive_qty = round_to_2_decimals(max(pushed_qty - completed_qty, 0))
        remaining_to_push = round_to_2_decimals(max(order_qty - pushed_qty, 0))
        if order.status == 'closed':
            status = 'closed'
        elif completed_qty + STOCK_COMPARE_EPSILON >= order_qty and order_qty > 0:
            status = 'completed'
        ...
        execution[item.id] = {
            ...
            'pending_receive_quantity': pending_receive_qty,
...
def build_purchase_order_todo_summary():
    ...
    summary['open_count'] = summary['pending_count'] + summary['partial_count']
    ...
    if user:
        ...
        if user.status == 'disabled':
            ...
            try:
                ...
                db.session.rollback()
                app.logger.error(f"Failed to unlock admin account: {e}")
...
def initialize_database():
    ...
    # 修复已有物料的空分类/单位
    try:
        default_unit = Unit.query.filter_by(name='个').first() or Unit.query.first()
        default_cat = MaterialCategory.query.filter_by(name='默认分类').first() or MaterialCategory.query.first()
        if default_unit:
            cnt = Material.query.filter(db.or_(Material.unit_id.is_(None), Material.unit_id == 0)).update(
                {Material.unit_id: default_unit.id}, synchronize_session=False)
            if cnt:
                app.logger.info(f'修复物料单位: {cnt} 条 → "{default_unit.name}"')
        if default_cat:
            cnt = Material.query.filter(db.or_(Material.category_id.is_(None), Material.category_id == 0)).update(
                {Material.category_id: default_cat.id}, synchronize_session=False)
            if cnt:
                app.logger.info(f'修复物料分类: {cnt} 条 → "{default_cat.name}"')
        db.session.commit()
    except Exception as e:
        db.session.rollback()
...
def is_safe_redirect_target(target):
    ...
    if test_url.netloc != ref_url.netloc:
        return False
    if test_url.path in {url_for('login'), url_for('logout')}:
        return False
    return True
...
# ==================== Operation ====================
def log_operation(operation_type, operation_content, target_type=None, target_id=None):
    """Write an operation log record."""
    try:
        ...
        log = OperationLog(
            ...
            operation_type=operation_type or '其他操作',
...
def index():
    ...
    purchase_order_summary = build_purchase_order_todo_summary()
    pending_stock_tasks = pending_in + pending_out + purchase_order_summary.get('open_count', 0) + low_stock
    today_in = InOrder.query.filter_by(date=date.today(), status='completed').count()
    today_out = OutOrder.query.filter_by(date=date.today(), status='completed').count()

    recent_in = InOrder.query.options(joinedload(InOrder.supplier)).order_by(InOrder.created_at.desc()).limit(5).all()
    recent_out = OutOrder.query.options(joinedload(OutOrder.department)).order_by(OutOrder.created_at.desc()).limit(5).all()
    recent_purchase = PurchaseRequest.query.order_by(PurchaseRequest.created_at.desc()).limit(5).all()

    in_progress_orders = (
        InOrder.query.options(joinedload(InOrder.supplier))
        .filter(InOrder.status == 'pending')
        .order_by(InOrder.created_at.desc(), InOrder.id.desc())
        .limit(5)
        .all()
    )
    out_progress_orders = (
        OutOrder.query.options(joinedload(OutOrder.department))
        .filter(OutOrder.status == 'pending')
        .order_by(OutOrder.created_at.desc(), OutOrder.id.desc())
        .limit(5)
        .all()
    )
    purchase_pending_orders = (
        PurchaseRequest.query.filter(PurchaseRequest.status == 'pending')
        .order_by(PurchaseRequest.created_at.desc(), PurchaseRequest.id.desc())
        .limit(5)
        .all()
    )
    purchase_order_pending_orders = (
    ...
    for order in recent_in[:3]:
        recent_work_items.append({
            ...
            'date': order.date,
            'status': order.status,
            'link': url_for('in_order_detail', id=order.id),
            'summary': order.supplier.name if order.supplier else '未选择供应商'
        })
    for order in recent_out[:3]:
        recent_work_items.append({
            'type': '领料单',
            ...
            'date': order.date,
            'status': order.status,
            'link': url_for('out_order_detail', id=order.id),
            'summary': (order.department.name if order.department else '') or order.purpose or '未填写领用对象'
        })
    for order in recent_purchase[:3]:
        recent_work_items.append({
            'type': '采购申请',
            'icon': 'bi-cart-plus',
            'badge': 'warning',
            'order_no': order.request_no,
            'date': order.date,
            'status': order.status,
            'link': url_for('purchase_request_detail', id=order.id),
            'summary': order.applicant or '未填写申请人'
        })
    recent_work_items.sort(key=lambda item: item['date'] or date.min, reverse=True)
    recent_work_items = recent_work_items[:6]
    ...
    return render_template('index.html',
                         ...
                         pending_stock_tasks=pending_stock_tasks,
                         ...
                         out_progress_orders=out_progress_orders,
                         purchase_pending_orders=purchase_pending_orders,
                         purchase_order_pending_orders=purchase_order_pending_orders,
...
def api_subcontract_quick_issue():
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(f"委外发料失败: {e}")
        return jsonify({'status': 'error', 'msg': '发料失败，请稍后重试'}), 500
...
def api_subcontract_quick_receive():
    ...
    # Add receive detail
    receive_item = SubcontractReceiveItem(
    ...
    if received_qty >= total_qty:
        ...
        order.status = 'partial' if order.status != 'processing' else 'processing'

    try:
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"委外收货失败: {e}")
        return jsonify({'status': 'error', 'msg': '收货失败，请稍后重试'}), 500
    ...
    for id in ids:
        ...
        if material:
            ...
            try:
                ...
                db.session.rollback()
                app.logger.error(f"删除物料 {id} 失败: {str(e)}")
                fail_count += 1
        else:
            fail_count += 1
...
def _ai_system_api_catalog():
    ...
    for rule in app.url_map.iter_rules():
        ...
        routes.append({
            ...
            'module': _ai_api_route_group(route),
        })
...
def _ai_system_api_catalog_response(message):
    ...
    routes = _ai_system_api_catalog()
    group_counts = []
    for group in AI_SYSTEM_API_GROUP_ORDER:
        count = sum(1 for item in routes if item['module'] == group)
        if count:
            group_counts.append(f'{group}{count}个')

    cards = []
    for item in routes:
        methods = ','.join(item['methods'])
        cards.append({
            'title': f'{methods} {item["route"]}',
            'meta': f'{item["module"]}；endpoint：{item["endpoint"]}',
        })

    reply = (
        f'能识别。下面是当前 C:\\wms 仓库管理系统运行时代码里已注册的 Flask API/接口路由，共 {len(routes)} 个。'
        f'按模块统计：{"、".join(group_counts)}。'
        '这些接口来自本机 app.url_map，不是大模型猜出来的。调用时注意：页面内接口通常需要先登录；'
        '原生入库、出库、盘点接口一般要先 POST /api/login 获取令牌；带 <id>、<code>、<path:data> 的路由要替换成真实参数。'
    )
...
def _ai_stock_shortage_help_response(message):
    compact = (message or '').strip().replace(' ', '')
    shortage_words = (
        '库存不足', '库存不够', '不够库存', '低库存', '负库存', '缺料', '缺货',
        '欠料', '没库存', '没有库存', '库存没了', '库存不够用', '不够用了',
        '没有了', '没了', '没货', '无货', '断货', '用完了', '没有料',
    )
    if not compact or not any(word in compact for word in shortage_words):
        return None

    mentioned_materials = _ai_find_materials_from_message(message, limit=5)
    ...
    cards = []
    seen_ids = set()
    for material in mentioned_materials + negatives + lows:
        if material.id in seen_ids:
            continue
        seen_ids.add(material.id)
        cards.append(_ai_material_payload(material))
        if len(cards) >= 8:
            break

    if mentioned_materials:
        first = mentioned_materials[0]
        stock, min_stock, safety_stock, alert_status = _material_alert_status_values(first)
        unit = first.unit.name if first.unit else ''
        reply = (
            f'已识别到物料 {first.code or ""} {first.name or ""} {first.spec or ""}，当前库存 {normalize_stock_quantity(stock)}{unit}，库存状态：{_ai_alert_status_label(alert_status)}。'
            '这种情况不要只问“怎么办”，按缺料处理：'
            '1. 先点库存查询确认该物料是否在其他仓库/库位还有可用库存；'
            '2. 查该物料流水，看是否有未完成入库、误出库或盘点差异；'
            '3. 如果只是实物已到但没入账，补做入库或盘点调整；'
            '4. 如果确实没有货，立即发起采购申请，并把物料编码、规格和需求数量写清楚；'
            '5. 采购到货后从采购订单下推入库，再安排领料。'
        )
    else:
        alert_status = (
            f'当前系统发现负库存 {len(negatives)} 项、低库存 {len(lows)} 项。'
            if inventory_alert_enabled()
            else f'当前系统发现负库存 {len(negatives)} 项；库存预警未启用，低库存按最低库存规则暂不统计。'
        )
        reply = (
            f'{alert_status}库存不足不要直接强行出库，建议按顺序处理：'
            '1. 先查库存确认物料编码、仓库/库位和实际可用数；'
            '2. 查最近入库、出库、盘点流水，排除单据未完成或账实差异；'
            '3. 有实物没入账就补做入库或盘点调整；'
            '4. 确实缺料就发起采购申请或采购订单，到货后再入库；'
            '5. 已经负库存的物料优先处理，避免继续领料扩大差异。'
        )

    material_search = (mentioned_materials[0].code or mentioned_materials[0].name or '') if mentioned_materials else ''
    stock_query_url = url_for('stock_query', search=material_search) if material_search else url_for('stock_query')
    material_list_url = url_for('material_list', search=material_search) if material_search else url_for('material_list', stock_filter='low')
    return _ai_json_response(
        reply,
        cards,
        [
            {'label': '库存查询', 'url': stock_query_url},
            {'label': '物料档案', 'url': material_list_url},
            {'label': '采购申请', 'url': url_for('purchase_request_add_page')},
            {'label': '新建入库单', 'url': url_for('in_order_add_page')},
        ],
    )
...
def _ai_purchase_order_receive_response(message, context=None):
    ...
    order = _ai_purchase_order_from_context_or_message(message, context)
    if not order:
        return _ai_json_response(
...
def _ai_usage_help_response(message):
    ...
    asks_help = any(word in compact for word in ('怎么', '如何', '不会用', '使用', '操作', '流程', '教程', '教我', '怎么做', '帮我'))
    if not asks_help:
        return None

    general_help = any(word in compact for word in ('不会用', '怎么用', '如何用', '使用系统', '用这个系统', '用这个软件', '管理软件', '教我'))

    stock_shortage_help = _ai_stock_shortage_help_response(message)
    if stock_shortage_help:
        return stock_shortage_help

    if any(word in compact for word in ('入库', '到货', '收货', '来料')):
        return _ai_json_response(
            '入库常用流程：1. 先维护物料和供应商；2. 如果是采购到货，先从采购订单下推入库；3. 录入入库明细、数量、仓库/库位；4. 保存后检查单据；5. 确认无误再完成入库，系统才会增加库存。',
            actions=[
                {'label': '新建入库单', 'url': url_for('in_order_add_page')},
                {'label': '打开入库列表', 'url': url_for('in_order_list')},
                {'label': '打开采购订单', 'url': url_for('purchase_order_list')},
            ],
        )

    if any(word in compact for word in ('领料', '出库', '发料', '销售发货')):
        return _ai_json_response(
            '领料/出库流程：1. 打开新建领料单；2. 选择领用部门、用途和仓库；3. 添加物料编码和数量；4. 保存后核对库存；5. 完成单据后系统扣减库存。也可以直接说“生成领料单 A001 20”，我会先生成草稿。',
            actions=[
                {'label': '新建领料单', 'url': url_for('out_order_add_page')},
                {'label': '打开领料列表', 'url': url_for('out_order_list')},
                {'label': '查询库存', 'url': url_for('stock_query')},
            ],
        )

    if any(word in compact for word in ('采购', '采购申请', '采购单', '采购订单')):
        return _ai_json_response(
            '采购流程：1. 先做采购申请；2. 审核后生成采购订单；3. 供应商到货后从采购订单下推入库；4. 入库完成后库存增加。这样可以追踪申请、订单、入库之间的来源关系。',
            actions=[
    ...
    if any(word in compact for word in ('扫码', '手机', 'app', 'APP', '条码')):
        return _ai_json_response(
            '手机扫码流程：1. 先下载手机扫码 APP；2. 手机和电脑/服务器在同一网络；3. 在 APP 里配置服务器地址；4. 选择入库、出库、盘点或查询模式；5. 扫码后核对数量再提交。',
            actions=[
                {'label': '下载扫码APP', 'url': url_for('mobile_app_download')},
                {'label': '手工入库', 'url': url_for('mobile_scan', mode='in')},
                {'label': '手工出库', 'url': url_for('mobile_scan', mode='out')},
                {'label': '库存查询', 'url': url_for('mobile_scan', mode='query')},
            ],
        )
    ...
    if general_help and not any(word in compact for word in ('入库', '到货', '收货', '来料', '领料', '出库', '发料', '采购', '扫码', '手机', 'app', 'APP', '条码', '盘点', '盘库', '物料', '基础资料', '供应商', '客户')):
        return _ai_json_response(
            '这个系统可以按“先建资料、再做单据、最后查报表”的顺序用：1. 先维护仓库、物料、供应商/客户；2. 采购先做采购申请和采购订单；3. 到货后做入库单；4. 生产或销售领用时做领料单；5. 每天看首页待处理、库存异常和库存查询。',
            actions=[
    ...
    if any(word in compact for word in ('物料', '基础资料', '供应商', '客户', '仓库')):
        return _ai_json_response(
            '基础资料建议先按这个顺序维护：1. 仓库；2. 物料分类和单位；3. 供应商/客户；4. 物料档案。物料编码建议保持唯一、稳定，后续出入库和扫码都依赖它。',
            actions=[
                {'label': '物料档案', 'url': url_for('material_list')},
                {'label': '仓库资料', 'url': url_for('warehouse_list')},
    ...
    return _ai_json_response(
        '你可以先按这条主线使用：先维护基础资料，再做采购申请和采购订单，到货后入库；生产或销售领用时做领料单；每天看首页待处理和库存异常。你也可以直接问我“怎么入库”“怎么领料”“怎么采购”“怎么扫码”。',
        actions=[
            {'label': '物料档案', 'url': url_for('material_list')},
            {'label': '采购申请', 'url': url_for('purchase_request_list')},
            {'label': '入库列表', 'url': url_for('in_order_list')},
            {'label': '领料列表', 'url': url_for('out_order_list')},
            {'label': '待处理中心', 'url': url_for('pending_documents')},
        ],
    )
...
def _ai_skill_catalog_response(message):
    ...
    if not compact:
        return None
    skill_words = ('技能', '能力', '会什么', '能做什么', '可以做什么', '你会干什么', '功能清单', '能力清单')
    if not any(word in compact for word in skill_words):
        return None

    cards = [
        {'title': skill['title'], 'meta': skill['description']}
        for skill in AI_LOCAL_SKILLS
        if skill['name'] != 'skill_catalog'
    ]
    return _ai_json_response(
        '我现在先按本地技能表处理确定性仓库问题，命中不了再交给大模型理解。已接入的本地技能如下，后续可以继续按业务场景扩展。',
        cards,
        [
...
AI_LOCAL_SKILLS = [
    ...
    },
    {
        ...
        'title': '库存短缺处理',
        'handler': _ai_stock_shortage_help_response,
        'description': '处理库存不足、低库存、负库存、缺料、缺货等问题。',
    },
    {
        'name': 'purchase_order_receive',
        'title': '采购单生成入库单',
        'handler': _ai_purchase_order_receive_response,
        'description': '在采购单详情页或指定采购单号时，生成采购入库单草稿。',
    },
    {
        'name': 'usage_help',
        'title': '业务操作导航',
        'handler': _ai_usage_help_response,
        'description': '回答入库、出库、采购、扫码、盘点、基础资料等怎么操作的问题。',
    },
    {
        'name': 'basic_conversation',
        'title': '基础对话和模型状态',
        'handler': _ai_basic_conversation_response,
        'description': '回答问候、模型状态、是否接入大模型等问题。',
    },
]
...
def _ai_try_local_skills(message, context=None):
    for skill in AI_LOCAL_SKILLS:
        try:
            response = skill['handler'](message, context)
        except TypeError:
            response = skill['handler'](message)
        if response:
...
def _ai_rule_based_response(message):
    ...
    if any(word in compact for word in ('今天', '今日')) and any(word in compact for word in ('出库', '领料', '发出', '领用', '用了')):
        reply, cards = _ai_today_issued_materials()
        return _ai_json_response(reply, cards, [{'label': '打开出库列表', 'url': url_for('out_order_list')}])

    if any(word in compact for word in ('今天', '今日')) and any(word in compact for word in ('到货', '到了', '入库', '到料', '来料', '收货')):
        reply, cards = _ai_today_received_materials()
        return _ai_json_response(reply, cards, [{'label': '打开入库列表', 'url': url_for('in_order_list')}])

    if any(word in compact for word in ('今天', '今日', '概况', '汇总', '看板')) and not any(word in compact for word in ('生成', '创建', '新增')):
    ...
    if any(word in compact for word in ('生成', '创建', '新增', '开一张', '做一张')):
        if any(word in compact for word in ('领料', '出库')):
    ...
    if found_orders and any(word in compact for word in ('单号', '单据', '打开', '查')):
        return _ai_json_response(
            f'找到 {len(found_orders)} 张相关单据，下面可以直接打开。',
            found_orders,
            [{'label': '全部待处理', 'url': url_for('pending_documents')}],
        )

    if any(word in compact for word in ('异常', '预警', '低库存', '负库存', '超储', '库存不足')):
        low_query = Material.query.options(joinedload(Material.unit)).filter(_material_low_stock_filter())
        negative_query = Material.query.options(joinedload(Material.unit)).filter(Material.stock < 0)
        lows = low_query.order_by(Material.stock.asc()).limit(8).all() if inventory_alert_enabled() else []
        negatives = negative_query.order_by(Material.stock.asc()).limit(8).all()
    ...
    if any(word in compact for word in ('待办', '待处理', '待入库', '待出库', '未完成', '待审核', '未审核')):
    ...
    if materials:
        ...
        if len(materials) == 1 and any(word in compact for word in ('流水', '明细', '变化', '记录', '最近')):
...
def _ai_llm_endpoint(overrides=None):
    ...
    base_url = override or get_system_setting(
        'ai_llm_base_url',
...
def _ai_normalize_image_attachments(raw_attachments):
    ...
    for item in raw_attachments[:3]:
        ...
        name = str(item.get('name') or '图片').strip()[:80]
        if not data_url.startswith('data:image/') or ';base64,' not in data_url:
            return [], '图片格式不正确，请上传 PNG、JPG、WEBP 或 GIF 图片。'
...
def _ai_prepare_vision_image(data_url, mime_type):
    try:
        ...
        return data_url, mime_type, '图片 base64 数据不正确，请重新上传。'

    if len(image_bytes) <= 350 * 1024 and mime_type in {'image/jpeg', 'image/png', 'image/webp'}:
...
def _ai_normalize_intent(raw):
    ...
    if intent not in AI_ASSISTANT_INTENTS:
        return None
...
def _ai_call_llm_intent(message, overrides=None):
    ...
    try:
        ...
        return _ai_normalize_intent(_ai_extract_json_object(content))
    except Exception as exc:
...
def _ai_call_llm_chat(message):
    ...
    system_prompt = (
        ...
        '如果用户提到物料编码、规格、单号、客户、供应商、数量，必须围绕这些信息回答，不要说“我没看懂”。'
        '涉及真实库存、单据、金额、客户、供应商、审核状态等系统数据时，不要编造；应建议用户用系统查询，或说明需要通过后台工具查询真实数据。'
        '库存不足/没货/缺料：建议先查库存和库位，再查流水，排除未完成单据或盘点差异；确实缺料则发起采购申请/采购订单，到货后入库。'
...
def _ai_call_llm_vision(message, images, context=None):
    ...
    if page_title or page_url:
        user_content.append({'type': 'text', 'text': f'当前系统页面：{page_title or "-"} {page_url or ""}'.strip()})
    for image in images:
...
DOCUMENT_NAVIGATION_MODULES = {
    'in_order': {
        ...
        'number': 'order_no',
        'date': 'date',
        'detail_url': '/in_order/{id}',
        'title': lambda item: _document_nav_related(item, 'supplier') or item.purpose or item.warehouse or '',
        'search': ['order_no', 'purpose', 'warehouse', 'remark', lambda item: _document_nav_related(item, 'supplier')],
    },
    'out_order': {
        'model': OutOrder,
        'number': 'order_no',
        'date': 'date',
        'detail_url': '/out_order/{id}',
        'title': lambda item: _document_nav_related(item, 'department') or item.customer or item.purpose or item.business_type or '',
        'search': ['order_no', 'customer', 'business_type', 'warehouse', 'purpose', 'remark', lambda item: _document_nav_related(item, 'department')],
    },
    'after_sale_out': {
        'model': AfterSaleOutOrder,
        'number': 'order_no',
        'date': 'date',
        'detail_url': '/after_sale_out/{id}',
        'title': lambda item: item.customer or item.reason or '',
        'search': ['order_no', 'customer', 'contact', 'phone', 'reason', 'remark'],
    },
    'purchase_request': {
        'model': PurchaseRequest,
        'number': 'request_no',
        'date': 'date',
        'detail_url': '/purchase_request/{id}',
        'title': lambda item: item.applicant or item.department or item.reason or '',
        'search': ['request_no', 'applicant', 'department', 'urgency', 'reason', 'remark'],
    },
    'transfer': {
        'model': TransferOrder,
        'number': 'transfer_no',
        'date': 'date',
        'detail_url': '/transfer/{id}',
        'title': lambda item: f'{item.from_location or ""} → {item.to_location or ""}'.strip(' →'),
        'search': ['transfer_no', 'from_location', 'to_location', 'remark'],
    },
    'adjustment': {
        'model': AdjustmentOrder,
        'number': 'adjustment_no',
        'date': 'date',
        'detail_url': '/adjustment/{id}',
        'title': lambda item: item.adjustment_type or item.source_type or '',
        'search': ['adjustment_no', 'adjustment_type', 'source_type', 'remark'],
    },
    'check': {
        ...
        'date': 'date',
        'detail_url': '/check/{id}',
        'title': lambda item: item.remark or '',
        'search': ['check_no', 'remark'],
    },
    'bom': {
        'model': BOM,
        'number': 'bom_no',
        'date': 'created_at',
        'detail_url': '/bom/{id}',
        'title': lambda item: f'{item.product_code or ""} {item.product_name or ""}'.strip(),
        'search': ['bom_no', 'product_code', 'product_name', 'version', 'remark'],
    },
    'requisition': {
        'model': ProductionRequisition,
        'number': 'req_no',
        'date': 'date',
        'detail_url': '/requisition/{id}',
        'title': lambda item: item.production_order or item.purpose or '',
        'search': ['req_no', 'production_order', 'purpose', 'remark'],
    },
    'subcontract': {
        'model': SubcontractOrder,
        'number': 'order_no',
        'date': 'date',
        'detail_url': '/subcontract/{id}',
        'title': lambda item: _document_nav_related(item, 'supplier') or item.contact or '',
        'search': ['order_no', 'contact', 'phone', 'remark', lambda item: _document_nav_related(item, 'supplier')],
    },
    'subcontract_issue': {
        'model': SubcontractIssue,
        'number': 'issue_no',
        'date': 'date',
        'detail_url': '/subcontract_issue/{id}',
        'title': lambda item: _document_nav_related(item, 'supplier') or _document_nav_related(item, 'subcontract_order', 'order_no') or '',
        'search': ['issue_no', 'remark', lambda item: _document_nav_related(item, 'supplier'), lambda item: _document_nav_related(item, 'subcontract_order', 'order_no')],
    },
    'subcontract_receive': {
        'model': SubcontractReceive,
        'number': 'receive_no',
        'date': 'date',
        'detail_url': '/subcontract_receive/{id}',
        'title': lambda item: _document_nav_related(item, 'supplier') or _document_nav_related(item, 'subcontract_order', 'order_no') or '',
...
def _document_nav_material_terms(item):
    terms = []
    for detail in getattr(item, 'items', []) or []:
        material = getattr(detail, 'material', None)
        if material:
            terms.extend([
                getattr(material, 'code', ''),
                getattr(material, 'name', ''),
                getattr(material, 'spec', ''),
            ])
        terms.extend([
            getattr(detail, 'material_code', ''),
            getattr(detail, 'material_name', ''),
            getattr(detail, 'spec', ''),
        ])
    return terms
...
PENDING_DOCUMENT_MODULES = [
    {
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'in_order_detail',
        'title': lambda item: _document_nav_related(item, 'supplier') or item.purpose or item.warehouse or '',
        'search': ['order_no', 'purpose', 'warehouse', 'remark', lambda item: _document_nav_related(item, 'supplier')],
        'action': '待审核/待完成',
        'icon': 'bi-box-arrow-in-down',
    },
    {
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'out_order_detail',
        'title': lambda item: _document_nav_related(item, 'department') or item.customer or item.purpose or item.business_type or '',
        'search': ['order_no', 'customer', 'business_type', 'warehouse', 'purpose', 'remark', lambda item: _document_nav_related(item, 'department')],
        'action': '待审核/待完成',
        'icon': 'bi-box-arrow-up',
    },
    {
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'after_sale_out_detail',
        'title': lambda item: item.customer or item.reason or '',
        'search': ['order_no', 'customer', 'contact', 'phone', 'reason', 'remark'],
        'action': '待完成',
        'icon': 'bi-box-arrow-up',
    },
    {
        ...
        'detail_endpoint': 'purchase_request_detail',
        'title': lambda item: item.applicant or item.department or item.reason or '',
        'search': ['request_no', 'applicant', 'department', 'urgency', 'reason', 'remark'],
        'action': lambda item: '待审批' if item.status == 'pending' else '待采购/待完成',
        'icon': 'bi-cart-plus',
    },
    {
        'key': 'transfer',
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'transfer_detail',
        'title': lambda item: f'{item.from_location or ""} → {item.to_location or ""}'.strip(' →'),
        'search': ['transfer_no', 'from_location', 'to_location', 'remark'],
        'action': '待处理',
        'icon': 'bi-arrow-left-right',
    },
    {
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'adjustment_detail',
        'title': lambda item: item.adjustment_type or item.source_type or '',
        'search': ['adjustment_no', 'adjustment_type', 'source_type', 'remark'],
        'action': '待处理',
        'icon': 'bi-sliders',
    ...
    },
    {
        ...
        'date': 'date',
        'status': ('pending',),
        'detail_endpoint': 'requisition_detail',
        'title': lambda item: item.production_order or item.purpose or '',
        'search': ['req_no', 'production_order', 'purpose', 'remark'],
        'action': '待工单领料',
        'icon': 'bi-file-earmark-text',
    },
    {
        ...
        'detail_endpoint': 'subcontract_detail',
        'title': lambda item: _document_nav_related(item, 'supplier') or item.contact or '',
        'search': ['order_no', 'contact', 'phone', 'remark', lambda item: _document_nav_related(item, 'supplier')],
        'action': lambda item: '待发料' if item.status == 'pending' else '加工中',
        'icon': 'bi-truck',
    },
]
...
def _pending_status_label(config, item):
    ...
    if status == 'pending':
        if config.get('key') in {'in_order', 'out_order'}:
            return '未审核/待完成'
        if config.get('key') == 'purchase_request':
            return '待审批'
        return '草稿/待处理'
    if status == 'approved':
        return '已审批'
...
def _pending_document_search_text(config, item):
    parts = [
        config.get('label'),
        _document_nav_value(item, config.get('number')),
        _format_document_nav_date(_document_nav_value(item, config.get('date'))),
        getattr(item, 'status', ''),
        _pending_action_label(config, item),
        config.get('title')(item) if callable(config.get('title')) else '',
    ]
    parts.extend(_document_status_terms(getattr(item, 'status', '')))
    for getter in config.get('search', []):
...
def _build_pending_document_rows(module_filter='', status_filter='', search=''):
    ...
    for config in PENDING_DOCUMENT_MODULES:
        if module_filter and config['key'] != module_filter:
            continue
        model = config['model']
        query = model.query.filter(model.status.in_(config['status']))
        if status_filter:
            query = query.filter(model.status == status_filter)
        query = query.order_by(model.created_at.desc(), model.id.desc())
        for item in query.all():
            if search_lower and search_lower not in _pending_document_search_text(config, item):
                continue
            item_date = _document_nav_value(item, config.get('date'))
            rows.append({
                'id': item.id,
                ...
                'date': item_date,
                'date_text': _format_document_nav_date(item_date),
                'title': config.get('title')(item) if callable(config.get('title')) else '',
                'status': item.status,
                'status_label': _pending_status_label(config, item),
                'action_label': _pending_action_label(config, item),
                'created_at': item.created_at,
                'detail_url': url_for(config['detail_endpoint'], id=item.id),
            })
...
def _document_status_terms(status):
    ...
    status_map = {
        'pending': ['pending', '未审核', '待审核', '待完成', '待处理', '草稿', '待审批'],
        'completed': ['completed', '已完成', '已审核', '已处理'],
        'approved': ['approved', '已审批', '已审核', '已批准'],
        'rejected': ['rejected', '已驳回', '驳回'],
        'processing': ['processing', '处理中', '加工中'],
...
def _status_from_search_keyword(keyword, allowed_statuses):
    ...
    for status in allowed_statuses:
        terms = {str(term).strip().lower() for term in _document_status_terms(status)}
...
def _get_order_list_filters(allowed_statuses, default_sort='created_at'):
    ...
    search = (request.args.get('search') or '').strip()
    date_start = parse_date_value(request.args.get('date_start'))
    date_end = parse_date_value(request.args.get('date_end'))
...
def _apply_in_order_search(query, search):
    ...
    conditions = [
        InOrder.order_no.like(search_like),
        InOrder.business_type.like(search_like),
        InOrder.purpose.like(search_like),
    ...
    if status_from_search:
        conditions.append(InOrder.status == status_from_search)
    return (
        query.outerjoin(Material, InOrderItem.material_id == Material.id)
        .outerjoin(Supplier, InOrder.supplier_id == Supplier.id)
        .outerjoin(PurchaseOrderItem, InOrderItem.source_purchase_order_item_id == PurchaseOrderItem.id)
        .outerjoin(PurchaseOrder, db.or_(
...
def _apply_out_order_search(query, search):
    ...
    status_from_search = _status_from_search_keyword(search, ('pending', 'completed'))
    conditions = [
        OutOrder.order_no.like(search_like),
        OutOrder.customer.like(search_like),
        OutOrder.purpose.like(search_like),
        OutOrder.business_type.like(search_like),
        Department.name.like(search_like),
        Material.code.like(search_like),
        Material.name.like(search_like),
        Material.spec.like(search_like),
    ]
    if status_from_search:
        conditions.append(OutOrder.status == status_from_search)
    return query.outerjoin(Material, OutOrderItem.material_id == Material.id).outerjoin(Department, OutOrder.department_id == Department.id).filter(
        db.or_(*conditions)
    )
...
def _apply_purchase_request_search(query, search):
    if not search:
        return query
    search_like = f'%{search}%'
    status_from_search = _status_from_search_keyword(search, ('pending', 'approved', 'rejected', 'completed'))
    conditions = [
        PurchaseRequest.request_no.like(search_like),
        PurchaseRequest.applicant.like(search_like),
        PurchaseRequest.department.like(search_like),
        PurchaseRequest.reason.like(search_like),
        PurchaseRequest.remark.like(search_like),
    ]
    if status_from_search:
        conditions.append(PurchaseRequest.status == status_from_search)
    return query.filter(
        db.or_(*conditions)
    )
...
def _apply_subcontract_search(query, search):
    if not search:
        return query
    search_like = f'%{search}%'
    status_from_search = _status_from_search_keyword(search, ('pending', 'processing', 'completed', 'cancelled'))
    conditions = [
        SubcontractOrder.order_no.like(search_like),
        SubcontractOrder.contact.like(search_like),
        SubcontractOrder.phone.like(search_like),
        SubcontractOrder.remark.like(search_like),
        Supplier.name.like(search_like),
        Material.code.like(search_like),
        Material.name.like(search_like),
        Material.spec.like(search_like),
    ...
    if status_from_search:
        conditions.append(SubcontractOrder.status == status_from_search)
...
def _apply_subcontract_issue_search(query, search):
    ...
    status_from_search = _status_from_search_keyword(search, ('pending', 'completed'))
    conditions = [
        SubcontractIssue.issue_no.like(search_like),
        SubcontractIssue.remark.like(search_like),
        SubcontractOrder.order_no.like(search_like),
        Supplier.name.like(search_like),
        Material.code.like(search_like),
        Material.name.like(search_like),
    ...
    if status_from_search:
        conditions.append(SubcontractIssue.status == status_from_search)
    return query.outerjoin(SubcontractOrder, SubcontractIssue.subcontract_order_id == SubcontractOrder.id).outerjoin(
        Supplier, SubcontractIssue.supplier_id == Supplier.id
    ).outerjoin(SubcontractIssueItem, SubcontractIssueItem.issue_id == SubcontractIssue.id).outerjoin(
...
def _apply_subcontract_receive_search(query, search):
    ...
    status_from_search = _status_from_search_keyword(search, ('pending', 'completed'))
    conditions = [
        SubcontractReceive.receive_no.like(search_like),
        SubcontractReceive.remark.like(search_like),
        SubcontractOrder.order_no.like(search_like),
        Supplier.name.like(search_like),
        Material.code.like(search_like),
        Material.name.like(search_like),
    ...
    if status_from_search:
        conditions.append(SubcontractReceive.status == status_from_search)
    return query.outerjoin(SubcontractOrder, SubcontractReceive.subcontract_order_id == SubcontractOrder.id).outerjoin(
        Supplier, SubcontractReceive.supplier_id == Supplier.id
    ).outerjoin(SubcontractReceiveItem, SubcontractReceiveItem.receive_id == SubcontractReceive.id).outerjoin(
...
def _status_from_master_keyword(keyword):
    ...
    inactive_terms = {'inactive', '停用', '禁用', '停用中'}
    if keyword in active_terms:
        return 'active'
    if keyword in inactive_terms:
        return 'inactive'
    return None
    ...
    try:
        ...
        with open(image_path, 'rb') as image_file:
            ...
            data = {
                ...
                'sender_wechat_id': config.sender_wechat_id or '',
                'receiver_name': config.receiver_name or '',
                'receiver_wechat_id': config.receiver_wechat_id or '',
                'receiver_search_key': config.receiver_search_key or config.receiver_name or '',
...
def _wechat_share_order(config, order, trigger_type='manual', force=False):
    ...
    ).filter(WechatShareLog.status.in_(('pending', 'sent'))).first()
    if existing and not force:
        return existing, 'skipped'

    image_bytes, original_filename = _build_order_share_image(order, 'in_order')
    folder = _wechat_share_output_dir()
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f'{timestamp}_{_wechat_share_safe_filename(order.order_no)}.png'
    ...
    log = WechatShareLog(
        ...
        receiver_wechat_id=config.receiver_wechat_id,
...
def run_wechat_share_for_today(trigger_type='manual', force=False, config=None):
    ...
    if not config.share_in_order:
        return {'status': 'skipped', 'msg': '未勾选入库单分享', 'created': 0, 'skipped': 0}

    orders = _wechat_share_today_in_orders()
    created = 0
    skipped = 0
    logs = []
    for order in orders:
    ...
    if not orders:
        return {'status': 'success', 'msg': '今天没有已完成入库单', 'created': 0, 'skipped': 0, 'logs': []}
    return {
        'status': 'success',
        'msg': f'已处理 {len(orders)} 张今日入库单，生成 {created} 张分享图片，跳过 {skipped} 张',
        'created': created,
        'skipped': skipped,
    ...
    for config in configs:
        ...
        try:
            ...
            marker = WechatShareLog(
                ...
                order_id=0,
                order_no=f'今日入库单 {date.today().strftime("%Y-%m-%d")}',
                share_date=date.today(),
                trigger_type='scheduled',
                status='sent' if result.get('created', 0) else 'skipped',
            ...
            db.session.commit()
            app.logger.info('微信分享定时任务已执行: config_id=%s time=%s', config.id, now_text)
        except Exception:
            db.session.rollback()
...
def _build_order_share_pdf(order, module_key):
    ...
    font_name = _share_pdf_font_name()
    output = io.BytesIO()
    doc = SimpleDocTemplate(
        ...
        topMargin=12 * mm,
        bottomMargin=12 * mm,
        title=f'{_share_pdf_text(order.order_no)} PDF'
    )

    base = ParagraphStyle('ShareBase', fontName=font_name, fontSize=9, leading=12)
    small = ParagraphStyle('ShareSmall', parent=base, fontSize=8, leading=10)
    ...
    right = ParagraphStyle('ShareRight', parent=base, alignment=TA_RIGHT)

    is_in_order = module_key == 'in_order'
    order_title = ('入库单：' if is_in_order else '领料单：') + _share_pdf_text(order.order_no)
    detail_title = '入库明细' if is_in_order else '领料明细'
    party_label = '供应商' if is_in_order else '领料部门'
    party_value = (
        order.supplier.name if is_in_order and getattr(order, 'supplier', None)
        else (order.customer or (order.department.name if getattr(order, 'department', None) else '-'))
    )
    business_label = '用途' if is_in_order else '业务类型'
    business_value = order.purpose if is_in_order else (order.business_type or order.purpose)
    date_label = '入库日期' if is_in_order else '领料日期'
    total_amount = sum((item.amount or 0) for item in order.items)
    total_qty = sum((item.quantity or 0) for item in order.items)

    story = [
        _share_pdf_paragraph(order_title, title_style),
        _share_pdf_paragraph(f'状态：{_share_pdf_status_label(order.status)}', right),
        Spacer(1, 4)
    ]

    info_rows = [
        ('单据编号', order.order_no, date_label, order.date, party_label, party_value),
        (business_label, business_value, '仓库', order.warehouse, '操作人', order.operator.username if order.operator else '-'),
        ('总金额', _share_pdf_money(total_amount), '备注', order.remark, '创建时间', order.created_at),
    ]
    info_data = []
    for row in info_rows:
        info_data.append([
            _share_pdf_paragraph(row[0], small),
            _share_pdf_paragraph(row[1], base),
            _share_pdf_paragraph(row[2], small),
            _share_pdf_paragraph(row[3], base),
            _share_pdf_paragraph(row[4], small),
            _share_pdf_paragraph(row[5], base),
        ])

    info_table = Table(info_data, colWidths=[22 * mm, 52 * mm, 22 * mm, 52 * mm, 22 * mm, 52 * mm])
    ...
    story.extend([
        _share_pdf_paragraph('单据信息', section_style),
        info_table,
        Spacer(1, 8),
        _share_pdf_paragraph(detail_title, section_style),
    ])

    headers = ['序', '物料编码', '物料名称', '规格', '单位', '当前库存', '数量', '单价', '金额']
    detail_data = [[_share_pdf_paragraph(label, base) for label in headers]]
    for index, item in enumerate(order.items, 1):
        material = item.material
        unit_name = material.unit.name if material and material.unit else '-'
        detail_data.append([
            _share_pdf_paragraph(index, base),
            _share_pdf_paragraph(material.code if material else '-', base),
            _share_pdf_paragraph(material.name if material else '-', base),
            _share_pdf_paragraph(material.spec if material else '-', base),
            _share_pdf_paragraph(unit_name, base),
            _share_pdf_paragraph(_share_pdf_qty(material.stock if material else 0), base),
            _share_pdf_paragraph(_share_pdf_qty(item.quantity), base),
            _share_pdf_paragraph(_share_pdf_money(item.price), base),
            _share_pdf_paragraph(_share_pdf_money(item.amount), base),
        ])
    if not order.items:
        detail_data.append([_share_pdf_paragraph('暂无明细', base)] + [''] * (len(headers) - 1))
    detail_data.append([
        '', '', '', '', '',
        _share_pdf_paragraph('合计', base),
        _share_pdf_paragraph(_share_pdf_qty(total_qty), base),
        '',
        _share_pdf_paragraph(_share_pdf_money(total_amount), base),
    ])

    detail_table = Table(
        detail_data,
        colWidths=[10 * mm, 26 * mm, 48 * mm, 42 * mm, 16 * mm, 25 * mm, 22 * mm, 24 * mm, 26 * mm],
        repeatRows=1
    )
    last_row = len(detail_data) - 1
    detail_table.setStyle(TableStyle([
        ('FONTNAME', (0, 0), (-1, -1), font_name),
        ('GRID', (0, 0), (-1, -1), 0.35, colors.HexColor('#d1d5db')),
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#eef2ff')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.HexColor('#111827')),
        ('BACKGROUND', (0, last_row), (-1, last_row), colors.HexColor('#fff7ed')),
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ...
    if not order.items:
        detail_table.setStyle(TableStyle([('SPAN', (0, 1), (-1, 1)), ('ALIGN', (0, 1), (-1, 1), 'CENTER')]))
...
def share_order_pdf(module_key, id):
    if module_key == 'in_order':
        ...
        return jsonify({'status': 'error', 'msg': '当前单据暂未支持PDF分享'}), 404
...
def _fit_image_text(draw, text, font, width):
    ...
    if _image_text_width(draw, text, font) <= width:
        return text
    ellipsis = '...'
    result = ''
    for ch in text:
        if _image_text_width(draw, result + ch + ellipsis, font) > width:
            break
        result += ch
...
def _wrap_image_text(draw, text, font, width, max_lines=2):
    ...
    current = ''
    for ch in text:
        if _image_text_width(draw, current + ch, font) <= width:
            current += ch
            continue
...
def _build_order_share_image(order, module_key):
    ...
    section_font = _share_image_font(22, bold=True)
    label_font = _share_image_font(18)
    value_font = _share_image_font(20, bold=True)
    small_font = _share_image_font(17)
    table_font = _share_image_font(18)
    table_bold = _share_image_font(18, bold=True)
    amount_font = _share_image_font(19, bold=True)

    probe = Image.new('RGB', (width, 200), 'white')
    draw = ImageDraw.Draw(probe)

    is_in_order = module_key == 'in_order'
    title = ('入库单：' if is_in_order else '领料单：') + _share_pdf_text(order.order_no)
    detail_title = '入库明细' if is_in_order else '领料明细'
    party_label = '供应商' if is_in_order else '领料部门'
    party_value = (
        order.supplier.name if is_in_order and getattr(order, 'supplier', None)
    ...
    col_widths = [48, 118, 210, 214, 66, 110, 92, 110, 120]
    headers = ['序', '物料编码', '物料名称', '规格', '单位', '当前库存', '数量', '单价', '金额']
    line_height = 24
    rows = []
    for index, item in enumerate(order.items, 1):
        ...
        raw_values = [
            ...
            unit_name,
            _share_pdf_qty(material.stock if material else 0),
            _share_pdf_qty(item.quantity),
            _share_pdf_money(item.price),
            _share_pdf_money(item.amount),
        ]
        wrapped = []
        for col_index, value in enumerate(raw_values):
            available = col_widths[col_index] - 16
            max_lines = 2 if col_index in (2, 3) else 1
            wrapped.append(_wrap_image_text(draw, value, table_font, available, max_lines=max_lines))
        row_height = max(54, 18 + max(len(lines) for lines in wrapped) * line_height)
        rows.append((wrapped, row_height))
    if not rows:
        rows.append(([['暂无明细']] + [['']] * 8, 58))

    header_height = 118
    info_height = 210
    table_height = 52 + sum(row[1] for row in rows) + 58
    footer_height = 54
    height = margin + header_height + 28 + info_height + 28 + table_height + footer_height

    image = Image.new('RGB', (width, height), '#f6f8fb')
    draw = ImageDraw.Draw(image)

    def rounded_box(x0, y0, x1, y1, fill, outline=None, radius=18, width_line=1):
        draw.rounded_rectangle((x0, y0, x1, y1), radius=radius, fill=fill, outline=outline, width=width_line)

    y = margin
    rounded_box(margin, y, width - margin, y + header_height, '#4338ca', radius=22)
    draw.text((margin + 28, y + 24), title, font=title_font, fill='white')
    status_text = _share_pdf_status_label(order.status)
    status_w = _image_text_width(draw, status_text, value_font) + 34
    draw.rounded_rectangle((width - margin - status_w - 28, y + 28, width - margin - 28, y + 64), radius=18, fill='#dcfce7')
    draw.text((width - margin - status_w - 11, y + 33), status_text, font=value_font, fill='#166534')
    meta = f'{date_label}：{_share_pdf_text(order.date)}    {party_label}：{_share_pdf_text(party_value)}    操作人：{_share_pdf_text(order.operator.username if order.operator else "-")}'
    draw.text((margin + 30, y + 78), _fit_image_text(draw, meta, label_font, content_width - 60), font=label_font, fill='#e0e7ff')
    y += header_height + 28

    rounded_box(margin, y, width - margin, y + info_height, 'white', '#dbe3ef', radius=18)
    draw.text((margin + 28, y + 22), '单据信息', font=section_font, fill='#111827')
    info = [
        ('单据编号', order.order_no),
        (date_label, order.date),
        (party_label, party_value),
        (business_label, business_value),
        ('仓库', order.warehouse),
        ('操作人', order.operator.username if order.operator else '-'),
        ('总金额', _share_pdf_money(total_amount)),
        ('备注', order.remark),
    ...
    for idx, (label, value) in enumerate(info):
        row = idx // 3
        col = idx % 3
        x = margin + 28 + int(col * cell_w)
        yy = grid_top + row * 44
        draw.text((x, yy), label, font=small_font, fill='#64748b')
        value_color = '#dc2626' if label == '总金额' else '#111827'
        draw.text((x, yy + 21), _fit_image_text(draw, value, value_font, int(cell_w) - 34), font=value_font, fill=value_color)
    y += info_height + 28

    rounded_box(margin, y, width - margin, y + table_height, 'white', '#dbe3ef', radius=18)
    draw.text((margin + 28, y + 18), f'{detail_title}  共 {len(order.items)} 项', font=section_font, fill='#111827')
    table_x = margin + 20
    table_y = y + 58
    table_w = sum(col_widths)
    draw.rectangle((table_x, table_y, table_x + table_w, table_y + 52), fill='#eef2ff', outline='#c7d2fe')
    x = table_x
    for idx, header in enumerate(headers):
        draw.text((x + 8, table_y + 15), header, font=table_bold, fill='#1f2937')
        draw.line((x, table_y, x, table_y + table_height - 58), fill='#dbe3ef')
        x += col_widths[idx]
    draw.line((table_x + table_w, table_y, table_x + table_w, y + table_height - 1), fill='#dbe3ef')

    row_y = table_y + 52
    for row_index, (wrapped, row_height) in enumerate(rows):
        fill = '#ffffff' if row_index % 2 == 0 else '#f8fafc'
        draw.rectangle((table_x, row_y, table_x + table_w, row_y + row_height), fill=fill, outline='#e5e7eb')
        x = table_x
        for col_index, lines in enumerate(wrapped):
            font = amount_font if col_index == 8 else table_font
            color = '#dc2626' if col_index == 8 else '#111827'
            text_x = x + 8
            if col_index in (0, 5, 6, 7, 8):
                line = lines[0] if lines else ''
                text_x = x + col_widths[col_index] - 8 - _image_text_width(draw, line, font)
            _draw_wrapped_text(draw, (text_x, row_y + 13), lines, font, color, line_height)
            x += col_widths[col_index]
        row_y += row_height

    draw.rectangle((table_x, row_y, table_x + table_w, row_y + 58), fill='#fff7ed', outline='#fdba74')
    total_label = '合计'
    draw.text((table_x + sum(col_widths[:5]) + 20, row_y + 18), total_label, font=table_bold, fill='#111827')
    qty_text = _share_pdf_qty(total_qty)
    amount_text = _share_pdf_money(total_amount)
    qty_x = table_x + sum(col_widths[:7]) - 8 - _image_text_width(draw, qty_text, table_bold)
    amount_x = table_x + table_w - 8 - _image_text_width(draw, amount_text, amount_font)
    draw.text((qty_x, row_y + 18), qty_text, font=table_bold, fill='#111827')
    draw.text((amount_x, row_y + 18), amount_text, font=amount_font, fill='#dc2626')
    y += table_height + 18

    footer = f'生成时间：{datetime.now().strftime("%Y-%m-%d %H:%M")}    仓库管理系统'
    draw.text((margin, y), footer, font=small_font, fill='#94a3b8')
...
def share_order_image(module_key, id):
    if module_key not in {'in_order', 'out_order'}:
        return jsonify({'status': 'error', 'msg': '当前单据暂未支持图片分享'}), 404
    order = _load_share_order(module_key, id)
...
def _wechat_helper_authorized():
    ...
    provided = (
        request.headers.get('X-Wechat-Helper-Token')
        or ''
    ).strip()
...
def _wechat_helper_task_payload(log):
    return {
        ...
        'receiver_name': log.receiver_name or '',
        'receiver_wechat_id': log.receiver_wechat_id or '',
        'receiver_search_key': (log.config.receiver_search_key if log.config else '') or log.receiver_name or log.receiver_wechat_id or '',
        'receiver_type': (log.config.receiver_type if log.config else '') or 'person',
        'auto_send': bool(log.config.auto_send if log.config else False),
...
def wechat_helper_task_image(log_id):
    ...
    if log.status != 'pending' or not log.image_path:
        abort(404)
...
@csrf.exempt
def wechat_helper_report_task(log_id):
    if not _wechat_helper_authorized():
        return jsonify({'status': 'error', 'msg': 'unauthorized'}), 401
    ...
    if helper_status == 'sent':
        ...
        log.message = message or '本机微信助手已发送'
    elif helper_status in {'ready', 'pending'}:
        log.status = 'pending'
        log.message = message or '已粘贴到微信，等待人工确认发送'
    else:
        log.status = 'failed'
        log.message = message or '本机微信助手发送失败'
    try:
        db.session.commit()
        return jsonify({'status': 'success', 'msg': 'reported'})
    except Exception as exc:
        db.session.rollback()
...
def _warehouse_query_from_args():
    ...
    if search:
        ...
        conditions = [
            Warehouse.code.like(f'%{search}%'),
            Warehouse.name.like(f'%{search}%'),
            Warehouse.type.like(f'%{search}%'),
            Warehouse.location.like(f'%{search}%'),
            Warehouse.remark.like(f'%{search}%'),
        ]
        if status_from_search:
            conditions.append(Warehouse.status == status_from_search)
        query = query.filter(db.or_(*conditions))
    if status_filter in ('active', 'inactive'):
        query = query.filter(Warehouse.status == status_filter)
...
def _department_query_from_args():
    ...
    if search:
        ...
        conditions = [
            ...
            Department.remark.like(f'%{search}%'),
        ]
        if status_from_search:
            conditions.append(Department.status == status_from_search)
        query = query.filter(db.or_(*conditions))
    if status_filter in ('active', 'inactive'):
...
def _bom_query_from_args():
    search, status_filter, sort_by, sort_order = _get_master_list_filters()
    allowed_sorts = {'bom_no', 'product_code', 'product_name', 'version', 'level', 'status', 'total_cost', 'created_at'}
    if sort_by not in allowed_sorts:
        sort_by = 'created_at'
    query = BOM.query.options(selectinload(BOM.items).joinedload(BOMItem.material))
    if search:
        search_like = f'%{search}%'
        status_from_search = _status_from_master_keyword(search)
        conditions = [
            BOM.bom_no.like(search_like),
            BOM.product_code.like(search_like),
            BOM.product_name.like(search_like),
            BOM.version.like(search_like),
            BOM.remark.like(search_like),
            Material.code.like(search_like),
            Material.name.like(search_like),
            Material.spec.like(search_like),
        ]
        if status_from_search:
            conditions.append(BOM.status == status_from_search)
        query = query.outerjoin(BOMItem, BOMItem.bom_id == BOM.id).outerjoin(Material, BOMItem.material_id == Material.id).filter(db.or_(*conditions)).distinct()
    if status_filter in ('active', 'inactive'):
...
def _print_template_query_from_args(model):
    ...
    if search:
        ...
        search_lower = search.lower()
        conditions = [
            model.name.like(search_like),
            model.template_type.like(search_like),
        ]
        if search_lower in {'默认', 'default'}:
            conditions.append(model.is_default.is_(True))
        if search_lower in {'普通', 'normal', '非默认'}:
            conditions.append(model.is_default.is_(False))
        if search_lower in {'excel', '表格', '电子表格'}:
            conditions.append(model.template_type == 'excel')
        if search_lower in {'html', '网页'}:
            conditions.append(model.template_type == 'html')
        query = query.filter(db.or_(*conditions))
    if status_filter == 'default':
        query = query.filter(model.is_default.is_(True))
    elif status_filter == 'normal':
        query = query.filter(model.is_default.is_(False))
    if type_filter:
        query = query.filter(model.template_type == type_filter)
    query, sort_by = _apply_master_order(query, model, sort_by, sort_order, allowed_sorts, 'created_at')
...
def _serialize_document_nav_item(item, config):
    ...
    item_date = _format_document_nav_date(_document_nav_value(item, config.get('date')))
    title = str(_document_nav_value(item, config.get('title')) or '')
    status = str(getattr(item, 'status', '') or '')
    detail_url = config.get('detail_url', '').format(id=item_id)
    return {
        'id': item_id,
        'no': number,
        'date': item_date,
...
def _document_nav_matches(item, row, config, keyword):
    if not keyword:
        return True
    parts = [row.get('no'), row.get('date'), row.get('title'), row.get('status'), str(row.get('id'))]
    parts.extend(_document_status_terms(row.get('status')))
    for getter in config.get('search', []):
        parts.append(_document_nav_value(item, getter))
    parts.extend(_document_nav_material_terms(item))
...
@login_required
def api_document_navigation(module_key):
    config = DOCUMENT_NAVIGATION_MODULES.get(module_key)
    if not config:
        return jsonify({'status': 'error', 'msg': '暂不支持该单据导航'}), 404

    current_id = request.args.get('current_id', type=int)
    keyword = (request.args.get('q') or '').strip().lower()
    items = config['model'].query.order_by(config['model'].id.asc()).all()
    rows = []
    for item in items:
        row = _serialize_document_nav_item(item, config)
        if _document_nav_matches(item, row, config, keyword):
            rows.append(row)
    ...
    return jsonify({
        'status': 'success',
        'current_id': current_id if current_index >= 0 else None,
        'first_id': ids[0] if ids else None,
        'prev_id': ids[current_index - 1] if current_index > 0 else None,
        'next_id': ids[current_index + 1] if 0 <= current_index < len(ids) - 1 else None,
...
def material_info_api():
    ...
    material_id = request.values.get('id', type=int)
...
DEFAULT_FIELD_CONFIGS = {
    'in_order_add': [
        {'key': 'code', 'label': '物料编码', 'width': 150, 'visible': True, 'sort': 1},
        {'key': 'name', 'label': '物料名称', 'width': 0, 'visible': True, 'sort': 2},
        {'key': 'spec', 'label': '规格', 'width': 100, 'visible': True, 'sort': 3},
        {'key': 'unit', 'label': '单位', 'width': 70, 'visible': True, 'sort': 4},
        {'key': 'stock', 'label': '当前库存', 'width': 90, 'visible': True, 'sort': 5},
        {'key': 'quantity', 'label': '数量', 'width': 100, 'visible': True, 'sort': 6},
        {'key': 'price', 'label': '单价', 'width': 100, 'visible': True, 'sort': 7},
        {'key': 'amount', 'label': '金额', 'width': 100, 'visible': True, 'sort': 8},
        {'key': 'action', 'label': '操作', 'width': 70, 'visible': True, 'sort': 9},
    ],
    'in_order_detail': [
        {'key': 'seq', 'label': '序', 'width': 50, 'visible': True, 'sort': 1},
        {'key': 'code', 'label': '物料编码', 'width': 0, 'visible': True, 'sort': 2},
        {'key': 'name', 'label': '物料名称', 'width': 0, 'visible': True, 'sort': 3},
        {'key': 'spec', 'label': '规格', 'width': 0, 'visible': True, 'sort': 4},
        {'key': 'unit', 'label': '单位', 'width': 0, 'visible': True, 'sort': 5},
        {'key': 'stock', 'label': '当前库存', 'width': 90, 'visible': True, 'sort': 6},
        {'key': 'quantity', 'label': '数量', 'width': 0, 'visible': True, 'sort': 7},
        {'key': 'price', 'label': '单价', 'width': 0, 'visible': True, 'sort': 8},
        {'key': 'amount', 'label': '金额', 'width': 0, 'visible': True, 'sort': 9},
        {'key': 'action', 'label': '操作', 'width': 120, 'visible': True, 'sort': 10},
    ],
    'out_order_add': [
        {'key': 'code', 'label': '物料编码', 'width': 150, 'visible': True, 'sort': 1},
        {'key': 'name', 'label': '物料名称', 'width': 0, 'visible': True, 'sort': 2},
    ...
    'check_detail': [
        ...
        {'key': 'spec', 'label': '规格', 'width': 0, 'visible': True, 'sort': 4},
        {'key': 'unit', 'label': '单位', 'width': 0, 'visible': True, 'sort': 5},
        {'key': 'stock_qty', 'label': '账面数量', 'width': 0, 'visible': True, 'sort': 6},
        {'key': 'check_qty', 'label': '盘点数量', 'width': 0, 'visible': True, 'sort': 7},
        {'key': 'diff_qty', 'label': '差异', 'width': 0, 'visible': True, 'sort': 8},
        {'key': 'action', 'label': '操作', 'width': 120, 'visible': True, 'sort': 9},
    ],
}
...
def save_user_field_config(user_id, page_type, configs):
    """保存用户的字段配置"""
    # 删除旧配置
    UserFieldConfig.query.filter_by(user_id=user_id, page_type=page_type).delete()
    
    # 添加新配置
    for cfg in configs:
        field_cfg = UserFieldConfig(
            user_id=user_id,
            page_type=page_type,
            field_key=cfg['key'],
            field_label=cfg.get('label', ''),
            width=cfg.get('width', 0),
            visible=cfg.get('visible', True),
            sort_order=cfg.get('sort', 0),
        )
        db.session.add(field_cfg)
...
@require_role('warehouse')
@login_required
def save_field_config_api(page_type):
    """保存字段配置API"""
    data = request.get_json(silent=True)
...
@app.route('/debug/in_order/<order_no>')
@login_required
def debug_in_order(order_no):
    """Disabled production debug endpoint."""
    abort(404)
...
def calculate_bom_cost_value(bom, refresh_unit_cost=True):
    total_cost = 0
    for item in bom.items:
        if refresh_unit_cost:
            item.unit_cost = (item.material.price if item.material else 0) or 0
        else:
            item.unit_cost = item.unit_cost or 0
        item.total_cost = round_to_2_decimals((item.quantity or 0) * item.unit_cost)
...
def _find_or_create_unit(name):
    ...
    if not name:
        return None
    unit = Unit.query.filter_by(name=name).first() or Unit.query.filter_by(code=name).first()
    if not unit:
        unit = Unit(code=name, name=name)
        db.session.add(unit)
        db.session.flush()
    return unit
...
def _find_or_create_supplier(name):
    ...
    if not name:
        return None
    supplier = Supplier.query.filter_by(name=name).first() or Supplier.query.filter_by(code=name).first()
    if not supplier:
        supplier = Supplier(code=name, name=name)
        db.session.add(supplier)
        db.session.flush()
    return supplier
...
def _find_or_create_customer(name, contact='', phone=''):
    ...
    if not name:
        return None
    customer = Customer.query.filter_by(name=name).first() or Customer.query.filter_by(code=name).first()
    if not customer:
        code_base = name if len(name) <= 50 else f'CUS-{Customer.query.count() + 1:03d}'
        code = code_base
        if Customer.query.filter_by(code=code).first():
            code = f'CUS-{Customer.query.count() + 1:03d}'
        customer = Customer(code=code, name=name, contact=contact or '', phone=phone or '')
        db.session.add(customer)
        db.session.flush()
    else:
...
def _find_or_create_warehouse(name):
    ...
    if not name:
        return None
    warehouse = Warehouse.query.filter_by(name=name).first() or Warehouse.query.filter_by(code=name).first()
    if not warehouse:
        seq = Warehouse.query.count() + 1
        code = f'WH{seq:03d}'
        while Warehouse.query.filter_by(code=code).first():
            seq += 1
            code = f'WH{seq:03d}'
        warehouse = Warehouse(code=code, name=name, status='active')
        db.session.add(warehouse)
        db.session.flush()
    return warehouse
...
def _find_or_create_material(code, name='', spec='', unit_name=''):
    ...
    if not material:
        ...
        if name and not material.name:
            material.name = name
        if spec and not material.spec:
            material.spec = spec
        if unit and not material.unit_id:
...
def _build_col_map(header_row, aliases):
    ...
    for idx, header in enumerate(header_row):
        ...
        for key, patterns in aliases.items():
            if key in col_map:
                continue
            if any(str(pattern).lower() in label for pattern in patterns):
                col_map[key] = idx
    return col_map
...
def _material_line_data(item, quantity=None, extra=None):
    ...
    data = {
        ...
        'quantity': float(quantity if quantity is not None else (getattr(item, 'quantity', 0) or 0)),
        'price': float(getattr(item, 'price', 0) or (material.price if material else 0) or 0),
...
def _material_row_common(item, quantity=None, price=None, amount=None, remark=None, extra=None):
    material = getattr(item, 'material', None)
    row = {
        'code': material.code if material else getattr(item, 'material_code', '') or '',
        'name': material.name if material else getattr(item, 'material_name', '') or '',
        'spec': material.spec if material else getattr(item, 'spec', '') or '',
        'unit': _material_unit_name(item),
        'quantity': quantity if quantity is not None else getattr(item, 'quantity', 0) or 0,
        'price': price if price is not None else getattr(item, 'price', 0) or 0,
        'amount': amount if amount is not None else getattr(item, 'amount', 0) or 0,
        'remark': remark if remark is not None else getattr(item, 'remark', '') or '',
    }
    if extra:
        row.update(extra)
    return row
...
def _render_generic_document_print(payload):
    ...
    payload.setdefault('total_quantity', sum(row.get('quantity', 0) or 0 for row in payload.get('rows', [])))
...
def _render_transfer_form(transfer=None):
    ...
    return render_template(
        ...
        doc_type='transfer',
        icon='bi-arrow-left-right',
        page_title='新增库存调拨单' if transfer is None else ('查看库存调拨单' if transfer.status != 'pending' else '编辑库存调拨单'),
        list_url=url_for('transfer_list'),
        save_url=url_for('save_transfer_table'),
        complete_url=url_for('complete_transfer', id=transfer.id if transfer else 0),
        detail_url='/transfer',
        readonly=bool(transfer and transfer.status != 'pending'),
        status=transfer.status if transfer else 'pending',
        order_id=transfer.id if transfer else '',
        order_no=transfer.transfer_no if transfer else generate_order_no('TF'),
        order_no_label='调拨单号',
        date_value=(transfer.date if transfer and transfer.date else date.today()).strftime('%Y-%m-%d'),
        header={
            'from_location': transfer.from_location if transfer else '',
            'to_location': transfer.to_location if transfer else '',
            'remark': transfer.remark if transfer else '',
        },
        ...
        can_revert=bool(transfer and transfer.status == 'completed'),
        can_save=not bool(transfer and transfer.status != 'pending'),
        complete_label='完成调拨',
        revert_label='反提交',
...
def _render_check_form(check=None):
    return render_template(
        ...
        save_url=url_for('save_check_table'),
        complete_url=url_for('complete_check', id=check.id if check else 0),
        detail_url='/check',
        readonly=bool(check and check.status != 'pending'),
        status=check.status if check else 'pending',
        order_id=check.id if check else '',
...
def _render_bom_form(bom=None):
    return render_template(
        ...
        doc_type='bom',
        icon='bi-diagram-3',
        page_title='新增BOM' if bom is None else '编辑BOM',
        list_url=url_for('bom_list'),
        save_url=url_for('save_bom_table'),
        complete_url='',
        detail_url='/bom',
        readonly=False,
        status=bom.status if bom else 'active',
        order_id=bom.id if bom else '',
        order_no=bom.bom_no if bom else generate_order_no('BOM'),
        order_no_label='BOM编号',
        date_value=date.today().strftime('%Y-%m-%d'),
        header={
            'product_code': bom.product_code if bom else '',
            'product_name': bom.product_name if bom else '',
            'version': bom.version if bom else 'V1.0',
            'status': bom.status if bom else 'active',
            'remark': bom.remark if bom else '',
        },
        materials=_document_materials_json(),
        units=_document_units_json(),
        warehouses=[],
        ...
        existing_items=[
            _material_line_data(item, extra={
                'price': float(item.unit_cost or (item.material.price if item.material else 0) or 0),
                'unit_cost': float(item.unit_cost or (item.material.price if item.material else 0) or 0),
                'total_cost': float(item.total_cost or 0),
                'usage': item.usage or '',
            }) for item in (bom.items if bom else [])
        ],
        can_complete=False,
        can_revert=False,
        can_save=True,
...
def _render_requisition_form(requisition=None):
    ...
    return render_template(
        ...
        icon='bi-file-earmark-text',
        page_title='新增工单领料单' if requisition is None else ('查看工单领料单' if requisition.status != 'pending' else '编辑工单领料单'),
        list_url=url_for('requisition_list'),
        save_url=url_for('save_requisition_table'),
        complete_url=url_for('complete_requisition', id=requisition.id if requisition else 0),
        detail_url='/requisition',
        readonly=bool(requisition and requisition.status != 'pending'),
        status=requisition.status if requisition else 'pending',
        order_id=requisition.id if requisition else '',
        order_no=requisition.req_no if requisition else generate_order_no('REQ'),
        order_no_label='工单领料单号',
        date_value=(requisition.date if requisition and requisition.date else date.today()).strftime('%Y-%m-%d'),
        header={
        ...
        can_revert=bool(requisition and requisition.status == 'completed'),
        can_save=not bool(requisition and requisition.status != 'pending'),
        complete_label='完成工单领料',
        revert_label='反提交',
...
def _serialize_adjustment_item_for_form(item):
    ...
    unit_name = ''
    if item.unit:
        unit_name = item.unit.name or ''
    elif material and material.unit:
        unit_name = material.unit.name or ''
    return {
        'id': item.id,
        'material_id': item.material_id,
        'code': material.code if material else '',
        ...
        'unit': unit_name,
        'stock': float(material.stock or 0) if material else 0,
        'quantity': abs(float(item.quantity or 0)),
...
def preview_out_order_template(id):
    ...
    rendered = render_template(
        'print_out_with_html.html' if template and template.template_type == 'html' else 'print_out.html',
        order=order,
        template=template,
...
def print_out_order_with_template(id):
    ...
    if template:
        ...
        if template.template_type == 'html':
            ...
            return render_template('print_out_with_html.html', order=order, template=template, rendered_content=rendered_content)
...
def api_purchase_order_todo_summary():
    summary = build_purchase_order_todo_summary()
    latest_orders = PurchaseOrder.query.options(joinedload(PurchaseOrder.supplier)).filter(
        PurchaseOrder.status.in_(('pending', 'partial'))
    ).order_by(
        PurchaseOrder.expected_date.asc().nullslast(),
        PurchaseOrder.date.asc(),
    ...
    return jsonify({
        ...
        'orders': [{
            ...
            'status_label': purchase_order_status_label(order.status),
            'is_overdue': bool(order.expected_date and order.expected_date < date.today()),
...
@login_required
def api_purchase_order_selectable():
    purchase_order_id = request.args.get('purchase_order_id', type=int) or 0
    supplier_id = request.args.get('supplier_id', type=int) or 0
    ...
    for order in query:
        ...
        order_match = not search or search in (order.order_no or '').lower() or search in supplier_name.lower()
        for item in order.items:
            remain_qty = round_to_2_decimals((item.quantity or 0) - (item.received_quantity or 0))
            ...
            row = {
                ...
                'unit': material.unit.name if material and material.unit else '',
                'quantity': round_to_2_decimals(item.quantity or 0),
                'received_quantity': round_to_2_decimals(item.received_quantity or 0),
                'remaining_quantity': remain_qty,
                'price': round_to_2_decimals(item.price or 0),
                'amount': round_to_2_decimals(remain_qty * (item.price or 0)),
            }
...
REPORT_DEFINITIONS = {
    ...
    'warehouse_monthly': {
        'title': '仓库月报表',
        'description': '按月份和物料汇总期初、入库、出库、期末结存和库存金额。',
        'icon': 'bi-calendar3',
        'accent': 'primary',
        'summary_labels': {
            'count': '月报行数',
    ...
    'purchase_order_execution': {
        'title': '采购订单执行统计表',
        'description': '按采购单和物料查看订购、已入库、未入库和逾期情况。',
        'icon': 'bi-bag-check',
        'accent': 'primary',
        'summary_labels': {
            'count': '明细行数',
    ...
    'supplier_purchase_summary': {
        'title': '供应商采购汇总表',
        'description': '按供应商汇总采购订单、数量、金额和未到货情况。',
        'icon': 'bi-building',
        'accent': 'success',
        'summary_labels': {
            'count': '供应商数',
    ...
    'material_purchase_summary': {
        'title': '物料采购汇总表',
        'description': '按物料编号、名称和规格汇总采购数量、金额和供应商数。',
        'icon': 'bi-box',
        'accent': 'info',
        'summary_labels': {
            'count': '物料数',
    ...
    'purchase_price_analysis': {
        'title': '采购价格分析表',
        'description': '按物料和供应商分析采购均价、最低价、最高价和最近采购价。',
...
REFERENCE_TYPE_LABELS = {
    ...
    'adjustment': '调整单',
    'inventory_check': '盘点单',
    'inventory_check_scan': '扫码盘点单',
    'stock_transaction': '库存流水',
    'after_sale_out_order': '售后出库单',
    'subcontract': '委外单',
    'subcontract_issue': '委外发料单',
    'subcontract_receive': '委外入库单',
    'requisition': '工单领料单',
}
REPORT_REFERENCE_LINKS = {
    'in_order': {'model': InOrder, 'number': 'order_no', 'endpoint': 'in_order_detail'},
    'out_order': {'model': OutOrder, 'number': 'order_no', 'endpoint': 'out_order_detail'},
    'after_sale_out_order': {'model': AfterSaleOutOrder, 'number': 'order_no', 'endpoint': 'after_sale_out_detail'},
    'inventory_check': {'model': InventoryCheck, 'number': 'check_no', 'endpoint': 'check_detail'},
    'inventory_check_scan': {'model': InventoryCheck, 'number': 'check_no', 'endpoint': 'check_detail'},
    'transfer': {'model': TransferOrder, 'number': 'transfer_no', 'endpoint': 'transfer_detail'},
    'adjustment': {'model': AdjustmentOrder, 'number': 'adjustment_no', 'endpoint': 'adjustment_detail'},
    'subcontract': {'model': SubcontractOrder, 'number': 'order_no', 'endpoint': 'subcontract_detail'},
    'subcontract_issue': {'model': SubcontractIssue, 'number': 'issue_no', 'endpoint': 'subcontract_issue_detail_fragment'},
    'subcontract_receive': {'model': SubcontractReceive, 'number': 'receive_no', 'endpoint': 'subcontract_receive_detail_fragment'},
...
def build_report_dashboard_context():
    ...
    stats = {
        ...
        'month_out_amount': float(month_out_amount),
        'month_in_count': int(month_in_count),
        'month_out_count': int(month_out_count),
    ...
    for offset in range(10):
        current_day = trend_start + timedelta(days=offset)
        trend_labels.append(current_day.strftime('%m-%d'))
        trend_in_qty.append(in_trend_map.get(current_day, 0))
        trend_out_qty.append(out_trend_map.get(current_day, 0))
    ...
    for name, count in category_rows:
        category_labels.append(name or 'Unnamed')
        category_values.append(int(count or 0))
    ...
    in_amount_map = {}
    out_amount_map = {}
    for row_date, amount in in_amount_rows:
        key = row_date.strftime('%Y-%m')
        in_amount_map[key] = in_amount_map.get(key, 0) + float(amount or 0)
    for row_date, amount in out_amount_rows:
        key = row_date.strftime('%Y-%m')
        out_amount_map[key] = out_amount_map.get(key, 0) + float(amount or 0)

    monthly_labels = []
    monthly_in_amount = []
    monthly_out_amount = []
    for offset in range(6):
        current_month = _shift_month_start(monthly_start, offset)
        key = current_month.strftime('%Y-%m')
        monthly_labels.append(key)
        monthly_in_amount.append(in_amount_map.get(key, 0))
        monthly_out_amount.append(out_amount_map.get(key, 0))

    chart_data = {
        'trend': {
            'labels': trend_labels,
            'in_qty': trend_in_qty,
            'out_qty': trend_out_qty,
        },
        'category': {
        ...
        'top_stock': {
            'labels': top_stock_labels,
            'values': top_stock_values,
        },
        'monthly_amount': {
            'labels': monthly_labels,
            'in_amount': monthly_in_amount,
            'out_amount': monthly_out_amount,
        },
    }
...
def _build_report_filters():
    return {
        ...
        'sort_field': (request.args.get('sort_field') or '').strip(),
...
def _material_matches(material, keyword):
    ...
    haystack = ' '.join(
        str(part or '').lower()
        for part in (material.code, material.name, material.spec)
    )
...
def _supplier_matches(supplier, keyword):
    ...
    haystack = ' '.join(
        str(part or '').lower()
        for part in (supplier.code, supplier.name)
    )
    return all(token.lower() in haystack for token in keyword.split() if token)
...
def _report_reference_info(reference_type, reference_id, fallback_no=''):
    ...
    fallback_no = fallback_no or ''
    if not config or not reference_id:
        return fallback_no, ''

    doc = db.session.get(config['model'], reference_id)
    if not doc:
        return fallback_no, ''
...
def _in_detail_columns():
    return [
        ...
        {'field': 'price', 'title': '单价', 'type': 'money'},
        {'field': 'amount', 'title': '金额', 'type': 'money'},
...
def _summary_columns():
    return [
        {'field': 'date', 'title': '日期'},
        {'field': 'in_count', 'title': '入库单数'},
        {'field': 'in_quantity', 'title': '入库数量'},
        {'field': 'in_amount', 'title': '入库金额', 'type': 'money'},
        {'field': 'out_count', 'title': '领料单数'},
        {'field': 'out_quantity', 'title': '领料数量'},
        {'field': 'out_amount', 'title': '领料金额', 'type': 'money'},
...
def _check_columns():
    return [
        {'field': 'date', 'title': '日期'},
        {'field': 'check_no', 'title': '盘点单号', 'link_field': 'order_url'},
        {'field': 'material_code', 'title': '物料编码'},
        {'field': 'material_name', 'title': '物料名称'},
        {'field': 'spec', 'title': '规格型号'},
        {'field': 'unit', 'title': '单位'},
        {'field': 'system_stock', 'title': '系统库存'},
        {'field': 'actual_stock', 'title': '实盘库存'},
...
def _warehouse_monthly_columns():
    return [
        ...
        {'field': 'spec', 'title': '规格型号'},
        {'field': 'unit', 'title': '单位'},
        {'field': 'beginning_quantity', 'title': '期初数量'},
        {'field': 'in_quantity', 'title': '入库数量'},
        {'field': 'out_quantity', 'title': '出库数量'},
        {'field': 'ending_quantity', 'title': '期末结存'},
        {'field': 'price', 'title': '单价', 'type': 'money'},
        {'field': 'in_amount', 'title': '入库金额', 'type': 'money'},
        {'field': 'out_amount', 'title': '出库金额', 'type': 'money'},
        {'field': 'inventory_amount', 'title': '库存金额', 'type': 'money'},
        {'field': 'document_count', 'title': '单据数量'},
...
def _purchase_order_execution_columns():
    return [
        {'field': 'date', 'title': '采购日期'},
        {'field': 'order_no', 'title': '采购单号', 'link_field': 'order_url'},
        {'field': 'supplier', 'title': '供应商'},
        {'field': 'material_code', 'title': '物料编码'},
        {'field': 'material_name', 'title': '物料名称'},
        {'field': 'spec', 'title': '规格型号'},
        {'field': 'unit', 'title': '单位'},
        {'field': 'order_quantity', 'title': '采购数量'},
        {'field': 'received_quantity', 'title': '已入库数量'},
        {'field': 'remaining_quantity', 'title': '未入库数量'},
        {'field': 'price', 'title': '单价', 'type': 'money'},
        {'field': 'amount', 'title': '采购金额', 'type': 'money'},
        {'field': 'received_amount', 'title': '已入库金额', 'type': 'money'},
        {'field': 'remaining_amount', 'title': '未入库金额', 'type': 'money'},
        {'field': 'expected_date', 'title': '预计到货'},
        {'field': 'overdue_days', 'title': '逾期天数'},
        {'field': 'status', 'title': '状态', 'type': 'status'},
        {'field': 'source_request_no', 'title': '来源申请'},
        {'field': 'remark', 'title': '备注'},
    ]
...
def _supplier_purchase_summary_columns():
    return [
        ...
        {'field': 'supplier', 'title': '供应商'},
        {'field': 'order_count', 'title': '采购单数'},
        {'field': 'material_count', 'title': '物料数'},
        {'field': 'order_quantity', 'title': '采购数量'},
        {'field': 'received_quantity', 'title': '已入库数量'},
        {'field': 'remaining_quantity', 'title': '未入库数量'},
        {'field': 'amount', 'title': '采购金额', 'type': 'money'},
        {'field': 'received_amount', 'title': '已入库金额', 'type': 'money'},
        {'field': 'remaining_amount', 'title': '未入库金额', 'type': 'money'},
        {'field': 'avg_price', 'title': '平均单价', 'type': 'money'},
...
def _material_purchase_summary_columns():
    return [
        ...
        {'field': 'spec', 'title': '规格型号'},
        {'field': 'unit', 'title': '单位'},
        {'field': 'supplier_count', 'title': '供应商数'},
        {'field': 'order_count', 'title': '采购单数'},
        {'field': 'order_quantity', 'title': '采购数量'},
        {'field': 'received_quantity', 'title': '已入库数量'},
        {'field': 'remaining_quantity', 'title': '未入库数量'},
        ...
        {'field': 'remaining_amount', 'title': '未入库金额', 'type': 'money'},
        {'field': 'avg_price', 'title': '平均单价', 'type': 'money'},
        {'field': 'last_supplier', 'title': '最近供应商'},
        {'field': 'last_purchase_date', 'title': '最近采购日期'},
    ]
...
def _purchase_price_analysis_columns():
    return [
        ...
        {'field': 'spec', 'title': '规格型号'},
        {'field': 'supplier', 'title': '供应商'},
        {'field': 'purchase_times', 'title': '采购次数'},
        {'field': 'quantity', 'title': '采购数量'},
        {'field': 'amount', 'title': '采购金额', 'type': 'money'},
        {'field': 'avg_price', 'title': '平均单价', 'type': 'money'},
        {'field': 'min_price', 'title': '最低单价', 'type': 'money'},
        {'field': 'max_price', 'title': '最高单价', 'type': 'money'},
        {'field': 'price_range', 'title': '价差', 'type': 'money'},
        {'field': 'last_price', 'title': '最近采购价', 'type': 'money'},
        {'field': 'last_purchase_date', 'title': '最近采购日期'},
...
def _collect_in_detail_rows(filters):
    ...
    if filters['end_date']:
        query = query.filter(InOrder.date <= filters['end_date'])
    if filters['supplier_id']:
...
def _collect_check_rows(filters):
    ...
    if filters['end_date']:
        query = query.filter(InventoryCheck.date <= filters['end_date'])
    if filters['status']:
...
def _collect_ledger_rows(filters):
    ...
    for transaction in transactions:
        ...
        reference_no, reference_url = _report_reference_info(
            transaction.reference_type,
            transaction.reference_id,
            f"{transaction.reference_type or 'txn'}-{transaction.reference_id or transaction.id}",
        )
        rows.append({
            ...
            'balance_quantity': after_balance,
            'material_id': material_id,
            'material_code': material.code or '',
            'material_name': material.name or '',
            'spec': material.spec or '',
            'quantity': _safe_float(transaction.quantity),
            'amount': _safe_float(transaction.quantity) * _safe_float(material.price),
...
def _purchase_order_item_query(filters):
    ...
    if filters.get('end_date'):
        query = query.filter(PurchaseOrder.date <= filters['end_date'])
...
def _build_purchase_summary_row(bucket):
    ...
    row = {
        ...
        if key not in ('order_ids', 'material_ids', 'supplier_ids')
    }
    return {
        **row,
        'order_count': len(bucket['order_ids']),
        'material_count': len(bucket.get('material_ids', set())),
        'supplier_count': len(bucket.get('supplier_ids', set())),
...
def _build_supplier_purchase_summary_report(filters):
    ...
    for row in _collect_purchase_order_execution_rows(filters):
        supplier_name = row['supplier'] or '未指定供应商'
        key = supplier_name
        bucket = rows_by_supplier.setdefault(key, {
            'supplier_code': '',
        ...
        if row['order_url']:
            bucket['order_ids'].add(row['order_url'])
        if row['material_code']:
            bucket['material_ids'].add(row['material_code'])
        bucket['order_quantity'] += _safe_float(row['order_quantity'])
        bucket['received_quantity'] += _safe_float(row['received_quantity'])
        bucket['remaining_quantity'] += _safe_float(row['remaining_quantity'])
        bucket['amount'] += _safe_float(row['amount'])
        bucket['received_amount'] += _safe_float(row['received_amount'])
        bucket['remaining_amount'] += _safe_float(row['remaining_amount'])
        if _safe_float(row['overdue_days']) > 0:
            bucket['overdue_count'] += 1
        row_date = parse_date_value(row['date'])
        if row_date and (bucket['last_purchase_date'] is None or row_date > bucket['last_purchase_date']):
            bucket['last_purchase_date'] = row_date

    supplier_by_name = {supplier.name: supplier for supplier in Supplier.query.all()}
    rows = []
    for bucket in rows_by_supplier.values():
        supplier = supplier_by_name.get(bucket['supplier'])
        if supplier:
            bucket['supplier_code'] = supplier.code or ''
...
def _build_material_purchase_summary_report(filters):
    rows_by_material = {}
    for row in _collect_purchase_order_execution_rows(filters):
        key = (row['material_code'], row['material_name'], row['spec'])
        bucket = rows_by_material.setdefault(key, {
        ...
        if row['order_url']:
            bucket['order_ids'].add(row['order_url'])
        if row['supplier']:
            bucket['supplier_ids'].add(row['supplier'])
        bucket['order_quantity'] += _safe_float(row['order_quantity'])
        bucket['received_quantity'] += _safe_float(row['received_quantity'])
        bucket['remaining_quantity'] += _safe_float(row['remaining_quantity'])
        bucket['amount'] += _safe_float(row['amount'])
...
def _build_purchase_price_analysis_report(filters):
    rows_by_key = {}
    for row in _collect_purchase_order_execution_rows(filters):
        key = (row['material_code'], row['material_name'], row['spec'], row['supplier'])
        bucket = rows_by_key.setdefault(key, {
            'material_code': row['material_code'],
            'material_name': row['material_name'],
            'spec': row['spec'],
            'supplier': row['supplier'] or '未指定供应商',
            'purchase_times': 0,
            'quantity': 0.0,
            'amount': 0.0,
            'min_price': None,
        ...
        quantity = _safe_float(row['order_quantity'])
        bucket['purchase_times'] += 1
        ...
        if row_date and (bucket['last_purchase_date'] is None or row_date >= bucket['last_purchase_date']):
            ...
            bucket['last_order_no'] = row['order_no']
            bucket['last_order_url'] = row.get('order_url', '')

    rows = []
    for bucket in rows_by_key.values():
        min_price = _safe_float(bucket['min_price'])
        max_price = _safe_float(bucket['max_price'])
        rows.append({
            **bucket,
            'min_price': min_price,
            'max_price': max_price,
            'price_range': round_to_2_decimals(max_price - min_price),
            'avg_price': round_to_2_decimals(bucket['amount'] / bucket['quantity']) if bucket['quantity'] else 0,
            'last_purchase_date': bucket['last_purchase_date'].isoformat() if bucket.get('last_purchase_date') else '',
        })
    return rows
...
def _build_inventory_report(filters):
    ...
    summary = {
        'count': len(rows),
        'quantity': sum(row['stock'] for row in rows),
...
def _build_summary_report(filters):
    daily = {}
    for row in _collect_in_detail_rows(filters):
        bucket = daily.setdefault(row['date'], {
            'date': row['date'],
            'in_count': 0,
            'in_quantity': 0.0,
            'in_amount': 0.0,
        ...
        bucket['in_count'] += 1
        bucket['in_quantity'] += _safe_float(row['quantity'])
        bucket['in_amount'] += _safe_float(row['amount'])

    supplier_active = filters.get('supplier_id') or filters.get('supplier')
    out_filters = filters if not supplier_active else {**filters, 'supplier_id': 0, 'supplier': ''}
    for row in ([] if supplier_active else _collect_out_detail_rows(out_filters)):
        bucket = daily.setdefault(row['date'], {
            'date': row['date'],
            'in_count': 0,
            'in_quantity': 0.0,
        ...
        bucket['out_count'] += 1
        bucket['out_quantity'] += _safe_float(row['quantity'])
        bucket['out_amount'] += _safe_float(row['amount'])
    ...
    summary = {
        'count': len(rows),
        'quantity': sum(row['in_quantity'] + row['out_quantity'] for row in rows),
        'amount': sum(row['in_amount'] + row['out_amount'] for row in rows),
    }
...
def _build_check_report(filters):
    ...
    summary = {
        'count': len(rows),
        'quantity': sum(row['actual_stock'] for row in rows),
...
def _build_ledger_report(filters):
    ...
    ending_balances = {}
    for row in rows:
        ending_balances[row.get('material_id')] = row['balance_quantity']
    summary = {
        'count': len(rows),
        'quantity': sum(row['in_quantity'] + row['out_quantity'] for row in rows),
        'amount': sum(ending_balances.values()),
    }
...
def _is_inbound_transaction(transaction_type, quantity):
    if transaction_type in ('opening', 'opening_stock'):
        return None
    if transaction_type in ('in', 'transfer_in', 'adjustment_in', 'check_in'):
        return True
...
def _build_warehouse_monthly_report(filters):
    ...
    material_query = Material.query.options(joinedload(Material.unit)).order_by(Material.code.asc(), Material.id.asc())
    if filters.get('supplier_id'):
        material_query = material_query.filter(Material.supplier_id == filters['supplier_id'])
    supplier_clause = _supplier_filter_clause(filters.get('supplier'))
    if supplier_clause is not None:
        material_query = material_query.join(Material.supplier).filter(supplier_clause)
    material_clause = _material_filter_clause(filters.get('material_code'))
    if material_clause is not None:
        material_query = material_query.filter(material_clause)
    ...
    for transaction in transactions:
        if not transaction.created_at:
            continue
        txn_month_start = transaction.created_at.date().replace(day=1)
        if txn_month_start < start_month or txn_month_start > end_month:
            continue
        key = (transaction.material_id, txn_month_start)
        ...
        if transaction.reference_type and transaction.reference_id:
            ...
            bucket['document_keys'].add(('stock_transaction', transaction.id))

    rows = []
    for month_start in reversed(list(_iter_month_starts(start_month, end_month))):
        for material in materials:
            bucket = month_buckets.get((material.id, month_start), {})
            in_quantity = _safe_float(bucket.get('in_quantity'))
            out_quantity = _safe_float(bucket.get('out_quantity'))
            ending_quantity = current_stock_by_material[material.id]
            beginning_quantity = ending_quantity - in_quantity + out_quantity
            price = _safe_float(material.price)
            rows.append({
                ...
                'spec': material.spec or '',
                'unit': material.unit.name if material.unit else '',
                'beginning_quantity': round_to_2_decimals(beginning_quantity),
                'in_quantity': round_to_2_decimals(in_quantity),
    ...
    summary = {
        'count': len(rows),
        'quantity': sum(row['ending_quantity'] for row in rows),
        'amount': sum(row['inventory_amount'] for row in rows),
    }
...
def _build_purchase_order_execution_report(filters):
    ...
    summary = {
        'count': len(rows),
        'quantity': sum(row['order_quantity'] for row in rows),
        'amount': sum(row['amount'] for row in rows),
    }
...
def _build_subcontract_report(filters):
    ...
    query = SubcontractOrder.query.options(
        joinedload(SubcontractOrder.supplier),
        selectinload(SubcontractOrder.items).joinedload(SubcontractItem.material),
        selectinload(SubcontractOrder.issue_orders).selectinload(SubcontractIssue.items).joinedload(SubcontractIssueItem.material),
        selectinload(SubcontractOrder.receive_orders).selectinload(SubcontractReceive.items).joinedload(SubcontractReceiveItem.material),
    )
    if filters.get('start_date'):
        query = query.filter(SubcontractOrder.date >= filters['start_date'])
    if filters.get('end_date'):
        query = query.filter(SubcontractOrder.date <= filters['end_date'])
    if filters.get('supplier_id'):
        query = query.filter(SubcontractOrder.supplier_id == filters['supplier_id'])
    supplier_clause = _supplier_filter_clause(filters.get('supplier'))
    if supplier_clause is not None:
    ...
    for order in orders:
        ...
        issue_items = [
            item
            for issue in (order.issue_orders or [])
            for item in (issue.items or [])
            if _material_matches(item.material, material_keyword)
        ]
        receive_items = [
            item
            for receive in (order.receive_orders or [])
            for item in (receive.items or [])
            if _material_matches(item.material, material_keyword)
        ]
        if material_keyword and not order_items and not issue_items and not receive_items:
            continue

        issue_qty = sum(_safe_float(item.quantity) for item in issue_items) if order.issue_orders else 0
        receive_qty = sum(_safe_float(item.quantity) for item in receive_items) if order.receive_orders else 0
        total_issue += issue_qty
        total_receive += receive_qty
        
        rows.append({
            'order_no': order.order_no,
            'order_url': _report_detail_url('subcontract_detail', order.id),
            'date': order.date.strftime('%Y-%m-%d') if order.date else '',
            'supplier': order.supplier.name if order.supplier else '',
...
def batch_update_alert_thresholds():
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(f'批量更新库存预警值失败: {e}')
...
def export_out_order_template():
    ...
    ws = wb.active
    ws.title = '领料单导入模板'
    ws.append(['单据编号', '日期', '用途', '领料部门', '物料编码', '物料名称', '规格', '单位', '数量', '单价', '金额', '备注'])
    ws.append(['CK20240101001', '2024-01-01', '领料单', '生产车间', 'MAT001', '示例物料', '规格A', '个', '100', '10.00', '1000.00', '示例备注'])
    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
