...
共扫描 **74** 条菜单链接，发现 **2** 条菜单文字与实际页面不一致。
...
| 2 | 采购订单 | `/purchase_order/add` | 200 | 新增采购订单 | - |
| 3 | 采购入库 | `/in_order/add` | 200 | 新增采购入库单 | - |
| 4 | 采购申请列表 | `/purchase_request` | 200 | 采购申请管理 | - |
| 5 | 采购订单列表 | `/purchase_order?view=list` | 200 | 采购单管理 | - |
| 6 | 采购入库明细 | `/in_order?type=purchase_in` | 200 | 采购入库明细 | - |
...
| 14 | 入库明细报表 | `/report/view/in_detail` | 200 | 入库明细报表 | - |
...
| 65 | 系统设置 | `/system_settings` | 200 | 系统设置 | - |
...
报告生成时间：Thu Jul 30 08:34:03 UTC 2026
