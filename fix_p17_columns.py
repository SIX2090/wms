# -*- coding: utf-8 -*-
"""一次性止血脚本：补齐 P1-7 采购退货出库缺失的三列（幂等，可重复执行）。

适用症状：
    sqlalchemy.exc.OperationalError: (sqlite3.OperationalError)
    no such column: out_order.source_in_order_id

根因：P1-7 给 out_order / out_order_item 新增的来源采购入库单列只加在
app.py auto_migrate_database() 里，而 start_wms_*.bat 默认
WMS_NO_DB_TOUCH=1 会整体跳过它，兜底的 app/fix_db_columns.py 只在
start_wms_offline.bat 里被调用（start_wms_auto.bat 连它都不跑），
存量库重启后补不上，访问出库单即 500。

用法（Windows，在 C:\\wms 目录下执行）：
    python fix_p17_columns.py                 # 自动探测数据库路径
    python fix_p17_columns.py C:\\wms\\app\\instance\\inventory.db   # 指定路径
...
def main() -> int:
    ...
    if not paths:
        ...
        print(r'    python fix_p17_columns.py C:\wms\app\instance\inventory.db')
        return 1
    total = 0
    for path in paths:
        print(f'▶ 处理数据库：{path}')
