...
# 用户编辑（/user/<id>/edit 用 request.form）
if uid:
    try:
        r = h.s.post(BASE+f'/user/{uid}/edit', data={'username': uname, 'name': f'测试用户{ts}改', 'role': 'warehouse'}, headers=hdr, timeout=25)
        h.rec('用户-编辑', r.status_code<400 and 'success' in r.text, f'HTTP {r.status_code} {r.text[:100]}')
    except Exception as e:
        h.rec('用户-编辑', False, f'EXC {e}')
    # 用户删除（/user/delete 用 form）
    try:
        r = h.s.post(BASE+'/user/delete', data={'ids[]': uid}, headers=hdr, timeout=25)
        h.rec('用户-删除', r.status_code<400, f'HTTP {r.status_code} {r.text[:100]}')
    except Exception as e:
        h.rec('用户-删除', False, f'EXC {e}')

# 审批中心页面 & 操作审计（页面无导出按钮，不测不存在的导出 URL）
