...
def main() -> None:
    ...
    with wms.app.app_context():
        ...
        check=wms.InventoryCheck(check_no="CHECK-STATE-001",date=date.today(),status="pending",warehouse="默认测试仓",operator_id=user.id); wms.db.session.add(check); wms.db.session.flush()
