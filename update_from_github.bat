...
for /f "delims=" %%s in ('git status --porcelain 2^>nul') do (
  ...
  REM Ignore dirty files that are not business code and should not block auto-update.
  REM   - update_from_github.bat / WMS_NO_DB_TOUCH.flag: produced by this updater
  REM   - apk_source/: third-party APK decompiled sources (androidx/retrofit2/okio etc.)
  REM     that change on every decompile and are not real business changes
  REM   - app/FETCH_HEAD: auto-generated by git fetch
  REM   - app/cd, app/git: accidental cmd artifacts
  REM   - app/.installed.flag: deployment/installation marker
  set "SKIP=0"
