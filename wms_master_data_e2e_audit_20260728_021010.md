...
> 报告生成时间：2026-07-28T02:08:52.967133  
> 审计耗时：1.29 秒  
...
| 10 | 用户账号 | `/user` | 13 | 11 | 2 | 0 | 2 | 0 |
| 11 | 系统设置 | `/system_settings` | 11 | 8 | 3 | 0 | 3 | 0 |
| 12 | 标签模板 | `/label_template` | 13 | 11 | 2 | 0 | 2 | 0 |
| 13 | BOM | `/bom` | 13 | 11 | 2 | 0 | 2 | 0 |
| 14 | 期初库存 | `/opening_stock` | 12 | 10 | 2 | 0 | 2 | 0 |
| 15 | 库存查询 | `/stock_query` | 11 | 11 | 0 | 0 | 0 | 0 |
| 16 | 批量打印标签 | `/label/batch_print` | 11 | 8 | 3 | 1 | 2 | 0 |
| 17 | 报表中心 | `/report` | 11 | 11 | 0 | 0 | 0 | 0 |
| 18 | 报表看板 | `/report/dashboard` | 11 | 11 | 0 | 0 | 0 | 0 |
| 19 | 批量导入 | `/batch_import` | 11 | 11 | 0 | 0 | 0 | 0 |
| 20 | 字典/自定义字段 | `/admin/console` | 11 | 10 | 1 | 0 | 1 | 0 |
...
| 8 | 5. 导入路由 | 存在 /import 路由 | 未找到 | ❌ N |
| 9 | 6. 导出路由 | 存在 /export 路由 | 未找到 | ❌ N |
...
| 4 | 1.3 列表页含表格 | HTML 含 <table> | 找到=True | ✅ Y |
| 5 | 1.4 搜索/分页参数 | 3 种参数组合均 200 | 通过 3/3 | ✅ Y |
| 6 | 2. 新增页可访问 | 存在 /add 路由 | 未找到 | ❌ N |
| 7 | 5. 导入路由 | 存在 /import 路由 | 未找到 | ❌ N |
| 8 | 6. 导出路由 | 存在 /export 路由 | 未找到 | ❌ N |
...
| 9 | 5. 导入路由 | 存在 /import 路由 | 未找到 | ❌ N |
| 10 | 6. 导出路由 | 存在 /export 路由 | 未找到 | ❌ N |
...
| 2 | 1.1 列表页含"新增"按钮 | HTML 含"新增"或"添加"文本 | 找到=False | ❌ N |
| 3 | 1.2 列表页含搜索框 | HTML 含搜索 input | 找到=False | ❌ N |
| 4 | 1.3 列表页含表格 | HTML 含 <table> | 找到=False | ❌ N |
| 5 | 1.4 搜索/分页参数 | 3 种参数组合均 200 | 通过 3/3 | ✅ Y |
| 6 | 2. 新增页 | 查询/工具页无需新增 | N/A | ✅ Y |
| 7 | 5. 导入路由 | 查询/工具页无需单独导入（统一走 /batch_import） | N/A | ✅ Y |
| 8 | 6. 导出路由 | 查询/工具页无需单独导出 | N/A | ✅ Y |
...
| 7 | 5. 导入路由 | 查询/工具页无需单独导入（统一走 /batch_import） | N/A | ✅ Y |
| 8 | 6. 导出路由 | 查询/工具页无需单独导出 | N/A | ✅ Y |
| 9 | 7. 权限：warehouse 可访问 | GET /admin/console 期望 200 | 实际 302 | ❌ N |
| 10 | 8. 权限：production 可读 | GET /admin/console 期望 200/302/403 | 实际 302 | ✅ Y |
| 11 | 10. 关联矩阵 | 无需 FK 校验 | OK | ✅ Y |
...
**总缺陷：15 个（P0=1, P1=14, P2=0）**

| # | 严重度 | 模块 | 路径 | 检查点 | 实测 | 修复建议 |
|---|--------|------|------|--------|------|----------|
| 1 | 🔴 P0 | #16 批量打印标签 | `/label/batch_print` | 1.3 列表页含表格 | 找到=False | 检查模板是否渲染了表格 |
| 2 | 🟠 P1 | #10 用户账号 | `/user` | 5. 导入路由 | 未找到 | 在 app.py 添加 /user/import 路由 |
| 3 | 🟠 P1 | #10 用户账号 | `/user` | 6. 导出路由 | 未找到 | 在 app.py 添加 /user/export 路由 |
| 4 | 🟠 P1 | #11 系统设置 | `/system_settings` | 2. 新增页可访问 | 未找到 | 在 app.py 添加 /system_settings/add 路由 |
| 5 | 🟠 P1 | #11 系统设置 | `/system_settings` | 5. 导入路由 | 未找到 | 在 app.py 添加 /system_settings/import 路由 |
| 6 | 🟠 P1 | #11 系统设置 | `/system_settings` | 6. 导出路由 | 未找到 | 在 app.py 添加 /system_settings/export 路由 |
| 7 | 🟠 P1 | #12 标签模板 | `/label_template` | 5. 导入路由 | 未找到 | 在 app.py 添加 /label_template/import 路由 |
| 8 | 🟠 P1 | #12 标签模板 | `/label_template` | 6. 导出路由 | 未找到 | 在 app.py 添加 /label_template/export 路由 |
| 9 | 🟠 P1 | #13 BOM | `/bom` | 5. 导入路由 | 未找到 | 在 app.py 添加 /bom/import 路由 |
| 10 | 🟠 P1 | #13 BOM | `/bom` | 6. 导出路由 | 未找到 | 在 app.py 添加 /bom/export 路由 |
| 11 | 🟠 P1 | #14 期初库存 | `/opening_stock` | 5. 导入路由 | 未找到 | 在 app.py 添加 /opening_stock/import 路由 |
| 12 | 🟠 P1 | #14 期初库存 | `/opening_stock` | 6. 导出路由 | 未找到 | 在 app.py 添加 /opening_stock/export 路由 |
| 13 | 🟠 P1 | #16 批量打印标签 | `/label/batch_print` | 1.1 列表页含"新增"按钮 | 找到=False | 在 toolbar 加新增按钮 |
| 14 | 🟠 P1 | #16 批量打印标签 | `/label/batch_print` | 1.2 列表页含搜索框 | 找到=False | 在 toolbar 加搜索框 |
| 15 | 🟠 P1 | #20 字典/自定义字段 | `/admin/console` | 7. 权限：warehouse 可访问 | 实际 302 | warehouse 角色访问 /admin/console 失败 |
...
- #11 系统设置 - 2. 新增页可访问: 未找到
- #16 批量打印标签 - 1.1 列表页含"新增"按钮: 找到=False
- #16 批量打印标签 - 1.2 列表页含搜索框: 找到=False

**权限边界** (1 项)

- #20 字典/自定义字段 - 7. 权限：warehouse 可访问: 实际 302
